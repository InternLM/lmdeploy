...
def generate_report(report_path: str):
    ...
    for dir_path in subfolders:
        second_subfolders = [f.path for f in os.scandir(dir_path) if f.is_dir()]
        for sec_dir_path in second_subfolders:
            print("-"*25, sec_dir_path.replace(report_path, ""), "-"*25)
            file.writelines("-"*25 + sec_dir_path.replace(report_path, "") + "-"*25 + '\n')
            
            benchmark_subfolders = [f.path for f in os.scandir(sec_dir_path) if f.is_dir()]
            for benchmark_subfolder in benchmark_subfolders:
                backend_subfolders = [f.path for f in os.scandir(benchmark_subfolder) if f.is_dir()]
                for backend_subfolder in backend_subfolders:
                    print("*"*10, backend_subfolder.replace(sec_dir_path, ""), "*"*10)
                    file.writelines("*"*10 + backend_subfolder.replace(sec_dir_path, "") + "*"*10 + '\n')
