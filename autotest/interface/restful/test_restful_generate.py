...
from utils.config_utils import get_workerid, get_turbomind_model_list
...
class _TestGenerateComprehensive:
    ...
    @pytest.mark.skipif(os.getenv("TEST_INPUT_IDS", "0") != "1", reason="Enable with TEST_INPUT_IDS=1")
    ...
    def test_stop_string(self):
        resp = self._post({
            "prompt": "Fruits: apple, banana, cherry",
            "stop": ["cherry"],
            "max_tokens": 20
        })
        text = resp.json()["text"]
    ...
    def test_streaming_incremental_correctness(self):
        ...
        raw_resp = requests.post(
            self.api_url,
            json={"prompt": prompt, "max_tokens": 5, "stream": True},
            headers=self.headers,
            stream=True,
            timeout=30
        )
