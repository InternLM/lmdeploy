...
@pytest.mark.gpu_num_2
@pytest.mark.pr_test
@pytest.mark.parametrize('run_config', [item for item in PYTORCH_PR_TEST_LLM_GPU2 if 'speculative-algorithm' not in item['extra_params'] ])
...
@pytest.mark.gpu_num_1
@pytest.mark.pr_test
@pytest.mark.parametrize('run_config', [item for item in PYTORCH_PR_TEST_LLM_GPU1 if 'speculative-algorithm' not in item['extra_params'] ])
