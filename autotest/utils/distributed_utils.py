...
def verify_service_functionality(host: str, api_port: int, model_name: str, check_count: int) -> bool:
    ...
    try:
        ...
        resp = requests.post(
            f"http://{host}:{api_port}/v1/chat/completions",
            json=test_data,
            timeout=15
        )
...
class RayLMDeployManager:
    ...
    def start_lmdeploy_api_server(self, model_path: str, model_param: dict):
        ...
        if self.is_master:
            ...
            log_path = os.path.join(self.log_dir, "lmdeploy_api.log")
            ...
            if self.health_check:
                ready = wait_for_model_service_ready(
            ...
            print(f"🔍 Worker node {self.node_rank} is verifying that the master node ({self.master_addr}:{self.api_port}) API Server is ready...")
            if self.health_check:
                ready = wait_for_model_service_ready(
                    host=self.master_addr,
                    api_port=self.api_port,
                ...
                if not ready:
                    raise RuntimeError(
                        f"Worker node {self.node_rank}: Master node API Server not ready within 1000 seconds, cannot continue"
                    )
            else:
...
def worker_node_wait(manager: RayLMDeployManager, timeout_minutes: int = 60):
    """
