...
def get_cmd(backend, engine_config, data_config):
    assert backend in ['turbomind', 'pytorch']

    model_path = engine_config.pop('model_path')
...
def benchmark_throughput(model_path, backend, engine_config, data_config):
    ...
    model_path = model_path or engine_config['model_path']
    ...
    try:
        ...
        for _data_config in data_config:
            ...
            cmd = get_cmd(backend, engine_config, _data_config)
...
def benchmark(model_path=None, backend=None, config_path=None):
    with open(config_path, 'r') as f:
        ...
        for engine_config in engine_configs:
            benchmark_throughput(model_path, backend, engine_config, data_config)
