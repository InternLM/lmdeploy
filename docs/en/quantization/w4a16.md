...
```{note}
The --search-scale option is relatively time-consuming, especially when --batch-size is set to 1 and both calib_samples and calib_seqlen are large. Therefore, we recommend users to employ this option when they are not concerned about the time it takes to quantize the model or when the precision of the quantized model is not a priority.
```
