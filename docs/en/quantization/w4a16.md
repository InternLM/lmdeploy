...
```{note}
--search-scale is time-consuming especially when batch_size==1, calib_samples and calib_seqlen are large. So, we recommend it when a user does not care about time consuming of search_scale or smooth quant model accuracy is not satisfying.
```
