...
`kv_cache_dtype`是 kv 量化和推理开关。

- `kv_cache_dtype='int4'` 代表 4bit k/v 量化和推理
- `kv_cache_dtype='int8'` 代表 8bit k/v 量化和推理
...
当启动 8bit k/v 推理时，需要修改参数 `kv_cache_dtype` 和 `use_context_fmha`。详细内容请查阅 [kv int8](../quantization/kv_quant.md) 部署文档。
