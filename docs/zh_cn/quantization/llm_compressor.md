...
本指南旨在介绍如何使用 LMDeploy 的 TurboMind 推理引擎，运行经由 [vllm-project/llm-compressor](https://github.com/vllm-project/llm-compressor) 工具进行 FP8 或 INT4 量化后的模型。
...
LMDeploy 也内置了通过 `llm-compressor` 对 Qwen3-30B-A3B 进行 FP8 和 INT4 量化的脚本，供大家进行参考。

### FP8 量化示例

LMDeploy 内置了通过 `llm-compressor-fp8` 对 Qwen3-30B-A3B 进行 FP8 量化的[脚本](https://github.com/InternLM/lmdeploy/blob/main/examples/lite/fp8/qwen3_30b_a3b_fp8.py)：
...
LMDeploy 内置了通过 `llm-compressor-int4` 对 Qwen3-30B-A3B 进行 AWQ 量化的[脚本](https://github.com/InternLM/lmdeploy/blob/main/examples/lite/int4/qwen3_30b_a3b_awq.py)：
...
量化后的模型可以通过 LMDeploy 进行离线推理，也可以通过 api_server 一键封装为服务，对外提供兼容 OpenAI 接口的 RESTful API。

### FP8 部署

#### 离线推理

通过以下几行简单的代码，可以对 FP8 量化后的模型实现离线批处理：
...
服务默认端口是 23333。在 server 启动后，你可以通过 OpenAI SDK 访问服务。关于服务的命令参数，以及访问服务的方式，可以阅读[这份](https://lmdeploy.readthedocs.io/zh-cn/latest/llm/api_server.html)文档。

### INT4 部署

#### 离线推理

通过以下几行简单的代码，可以对 INT4 量化后的模型实现离线批处理：
...
我们将 Qwen3-8B (Dense) 与 Qwen3-30B-A3B (MoE) 的 FP8 和 INT4 量化模型通过 LMDeploy 部署为服务，并使用 [opencompass](https://github.com/open-compass/opencompass) 在多个学术数据集上评测。

### FP8 精度评测

结果显示：Qwen3-8B 与 Qwen3-30B-A3B 的 FP8 量化模型精度与 BF16 模型差异不显著，精度符合预期。
...
结果显示：Qwen3-8B 的非对称量化整体优于对称量化，而 Qwen3-30B-A3B 在两种量化方式间差异不显著；Qwen3-8B 在对称/非对称量化下与 BF16 模型的精度差异小于 Qwen3-30B-A3B。与 BF16 相比，量化模型在长输出数据集，比如 aime2025 (平均 17,635 tokens)、LCB (平均 14,157 tokens)，精度下降更明显；在中短输出数据集，比如 ifeval (平均 1,885 tokens)，mmlu_pro (平均 2,826)，精度符合预期。
