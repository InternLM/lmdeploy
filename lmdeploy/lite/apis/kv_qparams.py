...
from lmdeploy.lite.utils import get_calib_loaders, collect_target_modules, memory_efficient_inference
from lmdeploy.lite.quantization import Observer

from transformers import AutoModel, AutoTokenizer

from transformers.models.llama.modeling_llama import LlamaAttention, LlamaForCausalLM, LlamaDecoderLayer
...
def stats_past_key_values(
    past_key_values: List[torch.Tensor],
    k_obs_list: List[Observer],
    v_obs_list: List[Observer],
    symmetry: bool
) -> None:
    """
    ...
    Args:
        past_key_values (List[Tensor]): Past key values generated by the model during forward pass.
        k_obs_list (List[Observer]): List of observers for collecting stats for keys.
        v_obs_list (List[Observer]): List of observers for collecting stats for values.
...
def main(
    ...
    assert granularity in ['per_tensor'], \
        'Currently, only per tensor quantization is supported for the kv cache.'
    assert bits == 8, \
        'Currently, only 8-bit quantization is supported for the kv cache.'
    ...
    calib_loader, _ = get_calib_loaders(calib_dataset, tokenizer, nsamples=calib_samples, seqlen=max_seq_len)
    ...
    if offload:
        import warnings
        warnings.warn(f"You are using the `offload` mode, in which the "
