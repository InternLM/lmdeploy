...
from lmdeploy.lite.utils import collect_target_modules, bimap_name_mod
from functools import partial
from pathlib import Path
from lmdeploy.lite.quantization.weight import WeightObserver, WeightQuantizer
from lmdeploy.lite.quantization.activation import KVCacheObserver, ActivationObserver
from lmdeploy.lite.utils import split_decoder_layer_inputs, concat_decoder_layer_outputs
from lmdeploy.lite.quantization.smooth import smooth_fc_fcs, smooth_ln_fcs, NORM_FCS_MAP, FC_FCS_MAP
...
class Calibration():
    ...
    Parameters:
      ...
      - work_dir: Directory path to save the calibration and quantization results
      - device: Device on which model is to be calibrated ('cpu' or 'cuda')
      
    Note: More detailed information should be added here to explain what this class does exactly and how it works. 
    """
    ...
    def __init__(self,
        ...
        if self.smooth:
            assert w_qconfig
            self.w_qconfig = Config.fromfile(w_qconfig)
    ...
    def _wrap_decoder_layers(self):
        """
        Method to wrap the decoder layers' forward functions for observing their key/value cache during batched forward passes.
        """
            ...
            for i in range(len(batch_args)):
                ...
                if k_obs and v_obs:
                    ...
                    batch_outputs.append(
                        self._ori_forwards[mod](*batch_args[i], **batch_kwargs[i]))
    ...
    def collect_outputs_stats(self):
        """
        Collect statistics (min, max, absmax values) of the observed outputs.
        Returns a dictionary with these collected stats.
        """
    ...
    def collect_kv_stats(self):
        """
        Collect statistics (min, max, absmax values) of the observed keys and values.
    ...
    def __enter__(self ):
        """
        Prepares the Calibration object for a 'with' statement by registering hooks and wrapping layer forward methods.
        """
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        """
        Clean up after a 'with' statement by removing registered hooks, restoring original forward methods,
        and if no exception occurred, collecting all gathered statistics and saving them.
        """
