...
from lmdeploy.lite.utils.cal_qparams import (
...
class WeightQuantizer(GlobalAvailMixin):
    ...
    Attributes:
        ...
        symmetry (bool): If True, use absmax scaling; if False, use min-max scaling.
        granularity (str): The granularity of quantization. Available options are
            'per_channel', 'per_tensor', and 'per_group'.
        group_size (Optional[int]): If using 'per_group' quantization, this is the
    ...
    def __init__(self,
        ...
        assert granularity in ['per_channel', 'per_tensor', 'per_group'], \
            "The 'granularity' argument must be one of 'per_channel', 'per_tensor', or 'per_group'."
        self.granularity = granularity

        if self.granularity == 'per_group':
            assert group_size > 0, "The 'group_size' argument must be greater than 0."
    ...
    def calculate_qparams(self, weight: torch.Tensor) -> QParams:
        ...
        Args:
            weight (torch.Tensor): The weight tensor with shape (out_features, in_features).
    ...
    def quant(self,
        ...
        Args:
            weight (torch.Tensor): The weight tensor with shape (out_features, in_features).
            qparams (Optional[QParams]): A namedtuple containing 'scales' and 'zero_points'.
