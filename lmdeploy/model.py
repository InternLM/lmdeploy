...
class Glm4Chat(BaseChatTemplate):
    ...
    def __init__(
            ...
            user='<|user|>',
            eoh='',
            assistant='<|assistant|>',  # 151337
            eoa='',
            separator='',
            stop_words=['<|user|>', '<|endoftext|>',
                        '<|observation|>'],  # 151336, 151329, 151338
            **kwargs):
        super().__init__(system=system,
                         meta_instruction=meta_instruction,
                         eosys=eosys,
    ...
    def get_prompt(self, prompt, sequence_start=True):
        ...
        if sequence_start:
            ...
            return f'{self.start}{meta}{self.user}\n{prompt}{self.assistant}'
        else:
            return f'\n{prompt}{self.assistant}'  # '<|user|>' generated by model
    ...
    def messages2prompt(self, messages, sequence_start=True):
        ...
        if self.meta_instruction is not None and sequence_start:
            if len(messages) and messages[0]['role'] != 'system':
                ret += f'{self.system}\n{self.meta_instruction}{self.eosys}'
        for message in messages:
            role = message['role']
            content = message['content']
            ret += f'{box_map[role]}\n{content}'
