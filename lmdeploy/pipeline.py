...
class Pipeline:
    ...
    def __init__(self,
                 model_path: str,
                 backend_config: Optional[TurbomindEngineConfig | PytorchEngineConfig] = None,
        ...
        try:
            ...
            self.limiter: Optional[asyncio.Semaphore] = None
    ...
    def infer(self,
              prompts: Union[str, Tuple[str, Any], List[str], List[dict], List[List[dict]], List[Tuple[str, Any]]],
    ...
    def chat(self,
             ...
             adapter_name: Optional[str] = None,
             **kwargs: Any) -> Union[Session, Iterator]:
        """Chat.
    ...
    def get_reward_score(self, input_ids: Union[List[int], List[List[int]]]) -> List[float]:
        """Get reward score.

        Args:
            input_ids: a list of token_id or a list of token_id list.
        ...
        if not isinstance(input_ids, list):
            raise TypeError(f'input_ids must be a list, got {type(input_ids)}')
        ...
        if isinstance(input_ids[0], int):
            input_ids = [input_ids]
        elif not all(isinstance(x, list) and all(isinstance(y, int) for y in x) for x in input_ids):
            raise TypeError('input_ids must be a list of integers or a list of lists of integers')
        ...
        try:
            ...
            logger.error(f'Failed to get reward score: {e}')
            raise
    ...
    def get_ppl(self, input_ids: Union[List[int], List[List[int]]]) -> List[float]:
        ...
        if not all(isinstance(x, list) and len(x) > 1 for x in input_ids):
            raise ValueError('Each input_ids sequence must have at least 2 tokens')
        ...
        try:
            ...
            logger.debug(f'sorted sizes: {sizes}')
            logger.debug(f'sorted indices: {indices}')

            for (start, end) in self._batch_iterator(sizes, max_input_len):
                logger.debug(f'processing batch: start={start}, end={end}')
                if start == end:
            ...
            logger.error(f'Failed to get perplexity: {e}')
            raise
