...
class ConceptChunkStateUpdateResult:
    ...
    concept_input_states: Tensor
    concept_update_mask: Tensor
...
class ConceptLMRuntimeOpsImpl(ABC):
    ...
    def _build_prefill_merge_layout(self, token_layout: _PrefillTokenLayout,
        ...
        merge_first_pos = torch.where(
            concept_seq_len < self.chunk_size,
            torch.zeros_like(concept_layout.local_ids),
            concept_layout.local_ids * self.chunk_size,
        )
        merge_last_pos = torch.where(
            concept_seq_len < self.chunk_size,
            (concept_seq_len - 1).clamp(min=0),
            concept_layout.local_ids * self.chunk_size + self.chunk_size - 1,
        )
        ...
        merge_short_concept_mask = concept_seq_len < self.chunk_size
