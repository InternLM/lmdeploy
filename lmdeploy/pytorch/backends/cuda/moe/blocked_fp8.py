...
class TritonFusedMoEBlockedF8Builder(FusedMoEBlockedF8Builder):
    ...
    def build(top_k: int,
        ...
        """Build from mlp."""
        if ep_size > 1:
            assert fp32_acc is False, 'FP32 MoE reduction is not supported by the DeepEP backend yet.'
            assert output_scale == 1.0, 'MoE output scaling is not supported by the DeepEP backend yet.'
            ...
            return FusedMoENormal(ep_size=1,
                                  ep_group=ep_group,
                                  num_experts=num_experts,
                                  hidden_dim=hidden_dim,
                                  renormalize=renormalize,
