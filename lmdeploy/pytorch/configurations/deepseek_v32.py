...
def _finalize_v32_cache_specs(model_config: ModelConfig, block_size: int):
    ...
    if model_config.num_layers == hf_config.num_hidden_layers and indexer_types:
        layer_ids = [
            layer_id for layer_id, indexer_type in enumerate(indexer_types) if indexer_type != 'shared'
        ]
    else:
    ...
    model_config.block_cache_specs = [
        BlockCacheSpec(DSA_INDEXER_K_CACHE_NAME, layer_ids,
                       dsa_packed_indexer_k_cache_shape(block_size, hf_config.index_head_dim), torch.uint8)
    ]
