...
class ExecutorBase:
    ...
    def _get_runtime_size(
        self, num_free_gpu_mem: int, cache_block_size: int, vocal_size: int
    ):
        ...
        while max_prefill_token_num > 0:
            ...
            num_available = (
                num_free_gpu_mem - runtime_cache_size
    ...
    def _adjust_block_size(self):
        ...
        if self.model_config.k_head_dim >= 512 and self.cache_config.block_size > 32:
            ...
            logger.warning(
                f"Update `block_size={self.cache_config.block_size}`"
    ...
    def update_configs(self):
        ...
        cache_block_size = CacheEngine.get_cache_block_size(
            cache_config.block_size, model_config, self.tp, cache_config.quant_policy
        )
        runtime_mem, max_prefill_token_num = self._get_runtime_size(
            free_mem, cache_block_size, vocal_size
        )
        if cache_config.max_prefill_token_num != max_prefill_token_num:
            ...
            logger.warning(
                f"No enough memory. Update max_prefill_token_num={max_prefill_token_num}"
            )
