...
@triton.jit
def _apply_rotary_impl(x_l, x_h, cos_l, cos_h, sin_l, sin_h, ENABLE_FP32_COMPUTE: tl.constexpr = False):
    ...
    if ENABLE_FP32_COMPUTE:
        # Match FlashAttention RoPE's FP32 multiply-add path.
        return x_l * cos_l - x_h * sin_l, x_h * cos_h + x_l * sin_h
...
def apply_rotary_pos_emb_qk_kernel(
    ...
    ENABLE_FP32_COMPUTE: tl.constexpr = False,
):
    ...
    q_elem_type = tl.float32 if ENABLE_FP32_COMPUTE else Q.dtype.element_ty
    ...
    if head_id < BLOCK_QH:
        ...
        q_h = tl.load(qh_ptrs)
        if ENABLE_FP32_COMPUTE:
            q_l, q_h = q_l.to(tl.float32), q_h.to(tl.float32)

        qe_l, qe_h = _apply_rotary_impl(q_l, q_h, cos_l, cos_h, sin_l, sin_h, ENABLE_FP32_COMPUTE)
        ...
        k_h = tl.load(kh_ptrs)
        if ENABLE_FP32_COMPUTE:
            k_l, k_h = k_l.to(tl.float32), k_h.to(tl.float32)

        ke_l, ke_h = _apply_rotary_impl(k_l, k_h, cos_l, cos_h, sin_l, sin_h, ENABLE_FP32_COMPUTE)
...
def apply_rotary_pos_emb(q: Tensor,
    ...
    Args:
        ...
        enable_fp32_compute (bool): Compute Q/K and table products in FP32,
            then cast on store. Defaults to False.
