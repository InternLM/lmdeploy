...
def _sparse_index_topk_byte_radix_kernel(top_k: int,
    ...
    def sparse_index_topk_byte_radix_kernel_(
        ...
        with T.Kernel(num_tokens, threads=threads) as row:
            ...
            state = T.alloc_shared((2,), T.int32)
            ...
            if seqlen <= top_k:
                ...
                while pos_emit_gt < seqlen:
                    ...
                    if key > threshold_key:
                        out_pos_buf[0] = T.atomic_add(state[0], 1, return_prev=True)
                ...
                while pos_emit_eq < seqlen:
                    ...
                    if key == threshold_key:
                        out_pos_buf[0] = gt_count + T.atomic_add(state[1], 1, return_prev=True)
                        if out_pos_buf[0] < top_k:
...
def sparse_index_topk(scores: torch.Tensor,
    ...
    The returned ids are packed but not score-sorted.  Sparse attention consumes them as a set of valid KV positions;
    avoiding final sorting is the point of this selector.
    """
    if not descending:
    ...
    if fill != _FILL:
        raise ValueError(f'sparse_index_topk only supports fill={_FILL}.')
    if not _is_power_of_two(k) or not is_sparse_index_topk_supported(k):
    ...
    _sparse_index_topk_byte_radix_kernel(k, _FILL, _THREADS)(scores, kv_seqlens, out)
    return out
