...
F16_T = tl.pointer_type(tl.float16)
@triton.autotune(
    configs=[
        ...
        Config({'BLOCK_M': 64, 'BLOCK_N': 128}, num_stages=2, num_warps=4),
        Config({'BLOCK_M': 128, 'BLOCK_N': 128}, num_stages=2, num_warps=8),
        Config({'BLOCK_M': 128, 'BLOCK_N': 128}, num_stages=3, num_warps=8),
        Config({'BLOCK_M': 128, 'BLOCK_N': 128}, num_stages=2, num_warps=8),
    ],
...
@triton.jit
def _fwd_kernel(
    Q, K_PAGE, V_PAGE, O, sm_scale,
    stride_qb, stride_qh, stride_qm, stride_qk,
    stride_kb, stride_kh, stride_kn, stride_kk,
    stride_vb, stride_vh, stride_vn, stride_vk,
    stride_ob, stride_oh, stride_om, stride_ok,
    nheads, seqlen_q , seqlen_kv , headdim,
    BATCH_SIZE, IS_CAUSUAL: tl.constexpr, MAX_CACHE: tl.constexpr,
    ...
    offs_k_page = off_b * MAX_CACHE + offs_n[None,:]
    offs_v_page = off_b * MAX_CACHE + offs_n[:,None]
    k_page_ptrs = K_PAGE + offs_k_page
    v_page_ptrs = V_PAGE + offs_v_page
    
    # Initialize pointers to Q, K, V, O
    q_ptrs = Q + off_b * stride_qb + off_h * stride_qh + (offs_m[:, None] * stride_qm + offs_d[None, :] * stride_qk)
    # k_ptrs = K_PAGE + off_b * stride_kb + off_h * stride_kh + (offs_n[ None,:] * stride_kn + offs_d[:,None] * stride_kk)
    # v_ptrs = K_PAGE + off_b * stride_vb + off_h * stride_vh + (offs_n[:, None] * stride_vn + offs_d[None, :] * stride_vk)
    ...
    for start_n in range(0, end , BLOCK_N):
        ...
        # -- compute qk ----
        k_ptrs = tl.load(k_page_ptrs).to(F16_T)
        v_ptrs = tl.load(v_page_ptrs).to(F16_T)
        # tl.device_print('k_ptrs', k_ptrs)
        k_ptrs += off_h * stride_kh + offs_d[:, None]
        v_ptrs += off_h * stride_vh + offs_d[None, :]
        # tl.static_print(k_ptrs)
        k = tl.load(k_ptrs)
        v = tl.load(v_ptrs)
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, k_page, v_page, is_causual):
        ...
        # k_page = torch.zeros(bs ,seqlen_kv,device=q.device, dtype=torch.int64)
        # v_page = torch.zeros(bs ,seqlen_kv,device=q.device, dtype=torch.int64)
        # offs_b = torch.arange(0,bs,device=q.device)
        # offs_m = torch.arange(0, seqlen_kv,device=q.device)
        
        # k_page += k.data_ptr() + offs_b[:,None]*k.stride(0)*2 + offs_m[None,:] * k.stride(1) * 2
        # v_page += v.data_ptr() + offs_b[:,None]*v.stride(0)*2 + offs_m[None,:] * v.stride(1) * 2

        _fwd_kernel[grid](
            q, k_page, v_page, o, 1/(Lk ** 0.5),
            q.stride(0), q.stride(2), q.stride(1), q.stride(3),
            k.stride(0), k.stride(2), k.stride(1), k.stride(3),
            v.stride(0), v.stride(2), v.stride(1), v.stride(3),
            o.stride(0), o.stride(2), o.stride(1), o.stride(3),
            num_heads, seqlen_q, seqlen_kv, q.shape[-1],
            bs,is_causual, k_page.shape[-1],
            BLOCK_DMODEL=Lk
        )
...
def bench_flash_attention(BATCH, H, N_CTX, D_HEAD, provider, dtype=torch.float16, device="cuda"):
    ...
    if provider == "triton":
        ...
        k_page = torch.zeros(BATCH ,N_CTX,device=q.device, dtype=torch.int64)
        v_page = torch.zeros(BATCH ,N_CTX,device=q.device, dtype=torch.int64)
        offs_b = torch.arange(0,BATCH,device=q.device)
        offs_m = torch.arange(0, N_CTX,device=q.device)
        
        k_page += k.data_ptr() + offs_b[:,None]*k.stride(0)*2 + offs_m[None,:] * k.stride(1) * 2
        v_page += v.data_ptr() + offs_b[:,None]*v.stride(0)*2 + offs_m[None,:] * v.stride(1) * 2

        sm_scale = 1.3
        fn = lambda: attention(q, k, v,k_page,v_page, IS_CAUSAL)
