...
class InternS2PreviewModel(Qwen3_5MoeModel):
    ...
    def __init__(self,
        ...
        self.use_preview_ts_encoder = getattr(config, 'ts_forecaster_config', None) is not None
        if self.use_preview_ts_encoder:
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                if self.use_preview_ts_encoder:
                    ts_embeds, ts_pad_mask, _ = self.time_series(ts_values, ts_lens, ts_sr, ts_channels)
...
class InternS2PreviewForConditionalGeneration(Qwen3_5MoeForConditionalGeneration):
    ...
    def _split_multimodal_inputs(context: StepContext):
        ...
        if context.input_multimodals is None:
            return vision_mm_inputs, ts_mm_inputs
    ...
    def _prepare_ts_inputs(self, input_ids: torch.Tensor, ts_mm_inputs: list[MultiModalData]):
    ...
    def prepare_inputs_for_generation(
        ...
        vision_mm_inputs, ts_mm_inputs = self._split_multimodal_inputs(context)
        ...
        # time series inputs
        ts_inputs = self._prepare_ts_inputs(input_ids, ts_mm_inputs)
