...
def e2e_pd_cluster(e2e_model, e2e_artifact_dir):
    ...
    port = _find_available_port()
    url = f"http://127.0.0.1:{port}"
    command = ["lmdeploy-router", "--host", "127.0.0.1", "--port", str(port), "--policy", "round_robin", "--lmdeploy-pd-disaggregation"]
    ...
    try:
        ...
        try:
            ...
            model_id = e2e_model
        yield SimpleNamespace(process=process, url=url, model=model_id, prefill_urls=prefill_urls, decode_urls=decode_urls)
    finally:
        _stop(process)
