...
class APIClient:
    ...
    def chat(self,
        ...
        Args:
            messages(List): the chat context, including history
        ...
        for outputs in self.chat_completions_v1(model=self.available_models[0],
                                                messages=messages,
            ...
            finish_reason = outputs['choices'][0]['finish_reason']
            content = outputs['choices'][0]['delta']['content']
...
def main(api_server_url: str = 'http://0.0.0.0:23333', session_id: int = 0, api_key: Optional[str] = None):
    ...
    if not api_server_url.startswith('http://'):
        print(f'[WARNING] api_server_url should start with "http://", but got "{api_server_url}"')  # noqa: E231
    ...
    while True:
        ...
        if prompt in ['exit', 'end']:
            ...
            response = []
            for text, finish_reason in api_client.chat(messages,
                                                       session_id=session_id,
            ...
            messages.append(dict(role='assistant', content=''.join(response)))
