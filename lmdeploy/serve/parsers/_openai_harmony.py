...
class GptOssResponseParser(ResponseParser):
    ...
    def _maybe_inject_tool_grammar(self) -> None:
        """Enforce tool calling and structured output with an xgrammar
        structural_tag attached to the request's ``response_format``.

        This is an internal mechanism: the tag reaches the engine through the
        regular ``response_format`` dict (the OpenAI protocol itself is not
        extended); a non-text ``response_format`` must be consumed or cleared
        here so it cannot conflict with Harmony tool-call constraints.

        Runs after :func:`normalize_chat_request`, so ``response_format`` is
        the normalized dict form. Two cases:

        - **Tool calling** (``tools`` present, ``tool_choice != 'none'``):
          builds a Harmony tool-call structural_tag via
          ``xgrammar.get_model_structural_tag("harmony", ...)``; any
          ``response_format`` is overridden (tool grammar takes priority).
        ...
        is_non_text = fmt is not None and fmt.get('type', 'text') != 'text'
    ...
    @staticmethod
    def _build_response_format_grammar(fmt: dict) -> dict | None:
        """Convert a normalized ``response_format`` dict (json_schema /
        regex_schema / json_object) into a Harmony-compatible structural_tag.
        ...
        if fmt_type == 'json_schema':
            ...
            content = JSONSchemaFormat(json_schema=raw if raw is not None else {'type': 'object'})
        elif fmt_type == 'regex_schema':
            content = RegexFormat(pattern=fmt.get('regex_schema') or '.*')
    ...
    def _convert_response_format_to_harmony(self) -> None:
        ...
        """
        fmt = getattr(self.request, 'response_format', None)
        if fmt is None or fmt.get('type', 'text') == 'text':
            return
        ...
        except Exception:  # fmt dumps or message manipulation may fail
    ...
    def _set_response_format(self, grammar: dict) -> None:
        ...
        if hasattr(self.request, 'model_copy'):
            self.request = self.request.model_copy(update={'response_format': grammar})
        else:
            self.request.response_format = grammar
