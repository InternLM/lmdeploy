...
class Status(BaseModel):
    ...
    models: list[str] | None = Field(default_factory=list, examples=[[]])
    unfinished: int = 0
...
async def _generate_response(node: 'Node', request_dict: dict, endpoint: str,
                             stream: bool) -> StreamingResponse | JSONResponse:
...
async def _handle_hybrid_request(request: ChatCompletionRequest | CompletionRequest,
                                 endpoint: str) -> StreamingResponse | JSONResponse:
...
async def _handle_distserve_prefill(request: ChatCompletionRequest | CompletionRequest,
                                    endpoint: str) -> tuple[dict, str, 'Node' | None]:
...
async def _ensure_pd_connection(p_url: str, d_url: str, with_error_handling: bool = False) -> JSONResponse | None:
...
async def _handle_distserve_decode(request: ChatCompletionRequest | CompletionRequest,
                                   endpoint: str,
                                   prefill_info: dict,
                                   p_url: str,
                                   ensure_connection: bool = False,
                                   handle_shelf: bool = False) -> StreamingResponse | JSONResponse:
...
async def _handle_request(request: ChatCompletionRequest | CompletionRequest,
                          endpoint: str,
                          is_chat: bool = False) -> StreamingResponse | JSONResponse:
