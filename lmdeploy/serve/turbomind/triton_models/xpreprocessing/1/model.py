...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        self.model_name = self.model_config['parameters']['model_name'][
            'string_value']

        # Parse model output configs and convert Triton types to numpy types
        input_names = ['INPUT_ID', 'REQUEST_INPUT_LEN', 'OFFSET']
        ...
        self.tokenizer = Tokenizer(
            osp.join(
                ...
                ['string_value']))
        self.xtok = XTOKENIZERS.get(self.model_name)(self.tokenizer)
    ...
    def execute(self, requests):
        ...
        for idx, request in enumerate(requests):
            ...
            query = pb_utils.get_input_tensor_by_name(request,
                                                      'QUERY').as_numpy()
            kwargs = pb_utils.get_input_tensor_by_name(request,
                                                       'KWARGS').as_numpy()
            kwargs = json.loads(kwargs[0][0])
            ...
            if len(kwargs) == 0:
                ...
                input_id, offset = self._query2ids(query[0][0].decode(),
                                                   **kwargs)
                request_input_len = np.ones(
                    (1, 1),
                    dtype=self.request_input_len_dtype) * input_id.shape[-1]
            ...
            offset_tensor = pb_utils.Tensor(
                'OFFSET',
                np.array(offset).astype(self.offset_dtype))
            ...
            inference_response = pb_utils.InferenceResponse(output_tensors=[
                input_id_tensor, request_input_len_tensor, offset_tensor
            ])
    ...
    def _query2ids(self, query, num_image, sequence_start, **kwargs):
        input_id, offset = self.xtok.query2ids((query, num_image),
                                               sequence_start=sequence_start)
        input_id = np.array(input_id)[None]
        offset = np.array(offset, dtype=np.int32)[None]
