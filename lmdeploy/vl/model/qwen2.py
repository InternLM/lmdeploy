...
class Qwen2VLModel(VisionModel):
    ...
    def proc_messages(self, messages, chat_template, add_bos, chat_template_kwargs=None):
    ...
    def to_pytorch(self, messages, chat_template, tokenizer, add_bos, chat_template_kwargs=None, **kwargs):
        ...
        prompt, IMAGE_TOKEN = self.proc_messages(messages, chat_template, add_bos, chat_template_kwargs)
    ...
    def to_turbomind(self, messages, chat_template, tokenizer, add_bos, chat_template_kwargs=None, **kwargs):
        prompt, IMAGE_TOKEN = self.proc_messages(messages, chat_template, add_bos, chat_template_kwargs)
        info = super().to_turbomind_aux(messages, prompt, IMAGE_TOKEN, tokenizer, add_bos)
        inputs = [x['content'] for x in messages if x['role'] == 'preprocess'][0]
