...
template <
    // dtype of Q/K/V/M
    typename Element_, typename ArchTag, int kQueriesPerBlock,
    int kKeysPerBlock_, int kSingleValueIteration_ = false>
struct LlamaAttentionKernel
    : AttentionKernel<Element_, ArchTag,
                      true, // isAligned_
                      kQueriesPerBlock, kKeysPerBlock_,
                      kSingleValueIteration_ // kSingleValueIteration_
                      > {
  using Base = AttentionKernel<Element_, ArchTag,
                               true, // isAligned_
                               kQueriesPerBlock, kKeysPerBlock_,
                               kSingleValueIteration_ // kSingleValueIteration_
                               >;
  ...
  static constexpr bool kNeedsOutputAccumulatorBuffer =
      Base::kNeedsOutputAccumulatorBuffer;
  ...
  struct Params : BaseParams {
    ...
    CUTLASS_DEVICE void
    update_batched_ptr(ptr_t &data_ptr, ptr_t *batch_seq_ptr,
                       int batch_seq_offset, int batch_id, int strideB) {
    ...
    CUTLASS_DEVICE bool advance_to_block() {
      ...
      if (cu_seqlens_q_ptr != nullptr) {
        ...
        if (!q_use_seqlens) {
          update_batched_ptr(query_ptr, q_batch_seqs_ptr, q_batch_seqs_offset,
                             batch_id, q_strideB);
          qq_start = 0;
        } else {
          qq_start = q_start;
        ...
        if (!o_use_seqlens) {
          update_batched_ptr(output_ptr, o_batch_seqs_ptr, o_batch_seqs_offset,
                             batch_id, o_strideB);
          qo_start = 0;
        } else {
          qo_start = q_start;
      ...
      if (cu_seqlens_k_ptr != nullptr) {
        ...
        update_batched_ptr(key_ptr, k_batch_seqs_ptr, k_batch_seqs_offset,
                           batch_id, k_strideB);
        update_batched_ptr(value_ptr, v_batch_seqs_ptr, v_batch_seqs_offset,
                           batch_id, v_strideB);
        k_start = 0;
      }
      ...
      output_ptr +=
          int64_t(qo_start + query_start) * o_strideM() + head_id * o_strideH;
  ...
  using BaseSharedStorageEpilogueAtEnd =
      typename Base::SharedStorageEpilogueAtEnd;
  using BaseSharedStorageEpilogueInLoop =
      typename Base::SharedStorageEpilogueInLoop;

  // TODO: find a way to optimize non aligned bias
  using BiasLoader = TileSmemLoader<
      scalar_t, cutlass::MatrixShape<kQueriesPerBlock, kKeysPerBlock>,
  ...
  using AccumLambdaIterator = typename DefaultMmaAccumLambdaIterator<
      typename MM0::Mma::Operator::IteratorC, accum_t, kWarpSize>::Iterator;
  ...
  using SharedStorage = typename cutlass::platform::conditional<
      kSingleValueIteration || kKeepOutputInRF, SharedStorageEpilogueAtEnd,
  ...
  static void CUTLASS_DEVICE attention_kernel(Params &p) {
    ...
    for (int32_t iter_key_start = 0; iter_key_start < p.num_keys;
      ...
      auto prologueV = [&](int blockN) {
        ...
        MM1::Mma::prologue(shared_storage.after_mm0.mm1.mm, iterator_V,
      ...
      typename MM0::IteratorA iterator_A(
          ...
          p.query_ptr, {problem_size_0_m, problem_size_0_k}, thread_id(),
          tb_offset_A);
      ...
      typename MM0::IteratorB iterator_B(
          ...
          {problem_size_0_k, problem_size_0_n}, thread_id(), tb_offset_B);

      auto my_warp_id = warp_id();
      auto my_lane_id = lane_id();

      // Construct thread-scoped matrix multiply
      typename MM0::Mma mma(shared_storage.mm0, thread_id(), my_warp_id,
                            my_lane_id);
      ...
      if (kSupportsBias && p.attn_bias_ptr != nullptr) {
        ...
        typename BiasLoader::GmemTileIterator bias_iter(
            ...
            {problem_size_0_m, problem_size_0_n}, thread_id());
            ...
            DISPATCH_BOOL(
                ...
                ([&] {
                  ...
                  MM0::ScalingCoefsUpdater::update<kQueriesPerBlock,
                                                   kFullColumns, kIsFirst,
                                                   kKeepOutputInRF>(
                      accum_o, accum, mi, m_prime, s_prime, lane_id(),
                      thread_id(), warp_id(), p.num_keys - iter_key_start,
                      iteratorC_tile_offset, kSupportsBias ? 1.0f : p.scale);
                }));
          }));
      ...
      MM0::B2bGemm::accumToSmem(shared_storage.after_mm0.si, accum, my_lane_id,
                                output_tile_coords);
      ...
      for (int blockN = 0; blockN < nBlockN; ++blockN) {
        ...
        typename MM1::Mma mma_pv(shared_storage.after_mm0.mm1.mm,
                                 shared_storage.after_mm0.si, (int)thread_id(),
                    ...
                    ([&] {
                      ...
                      using EpilogueOutputOp = typename cutlass::epilogue::
                          thread::MemoryEfficientAttentionNormalize<
                              typename cutlass::platform::conditional<
                                  kIsLast, output_t, output_accum_t>::type,
                              output_accum_t, DefaultOp::kCount,
                              typename DefaultOp::ElementAccumulator,
                              ElementCompute, kIsFirst, kIsLast,
                      ...
                      using Epilogue = typename cutlass::epilogue::threadblock::
                          EpiloguePipelined<
                              ...
                              typename cutlass::platform::conditional<
                                  kIsLast, typename MM1::OutputTileIterator,
                      ...
                      auto dest_iter =
                          call_conditional<kIsLast, decltype(createOutputIter),
                                           decltype(createOutputAccumIter)>::
                              apply(createOutputIter, createOutputAccumIter,
                                    col);
                      EpilogueOutputOp rescale(s_prime, m_prime);
                      Epilogue epilogue(
                          shared_storage.epilogue_shared_storage(), thread_id(),
                          warp_id(), lane_id());
  ...
  static CUTLASS_DEVICE int8_t lane_id() { return Base::lane_id(); }
  static CUTLASS_DEVICE int8_t warp_id() { return Base::warp_id(); }
  static CUTLASS_DEVICE int16_t thread_id() { return Base::thread_id(); }
};

template <typename T, typename Attention>
void invokeFlashAttention_impl(
    int batch_size, int head_num, int key_len, int seq_len, int size_per_head,
    typename FlashAttentionOp<T>::Params &attention_params, cudaStream_t st) {
  ...
  using scalar_t = typename std::conditional_t<
      std::is_same<half, typename std::decay<T>::type>::value, cutlass::half_t,
      T>;

  const float qk_scale = static_cast<float>(1.f / sqrtf(size_per_head * 1.f));

  constexpr bool kNeedsOutputAccumulatorBuffer =
  ...
  typename Attention::Params params{};
  {
    ...
    params.output_accum_ptr =
        kNeedsOutputAccumulatorBuffer ? output_accum_ptr : nullptr;
  ...
  if (smem_bytes > 0xc000) {
    cudaFuncSetAttribute(attention_kernel_batched_impl<Attention>,
                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                         smem_bytes);
  }

  attention_kernel_batched_impl<Attention>
      <<<block_grid, thread_grid, smem_bytes, st>>>(params);
}

template <typename T> class FlashAttentionOp<T>::impl {
...
private:
  ...
  using scalar_t = typename std::conditional_t<
      std::is_same<half, typename std::decay<T>::type>::value, cutlass::half_t,
      T>;
  using SingleValueAttention =
      LlamaAttentionKernel<scalar_t, ArchTag, kQueriesPerBlock, kKeysPerBlock,
                           true>;
  using MultiValueAttention =
      LlamaAttentionKernel<scalar_t, ArchTag, kQueriesPerBlock, kKeysPerBlock,
                           false>;
...
public:
  impl(int batch_size, int head_num, int key_len, int seq_len,
       int size_per_head)
      : batch_size_(batch_size), head_num_(head_num), key_len_(key_len),
        seq_len_(seq_len), size_per_head_(size_per_head) {
  ...
  int get_workspace_size() const {
    if (kSingleValueIteration) {
      ...
      constexpr bool kNeedsOutputAccumulatorBuffer =
          MultiValueAttention::kNeedsOutputAccumulatorBuffer;
      if (kNeedsOutputAccumulatorBuffer) {
        return batch_size_ * head_num_ * seq_len_ * size_per_head_ *
               sizeof(float);
      } else {
        return 0;
      }
  ...
  void operator()(Params &params, cudaStream_t st) const {
    if (kSingleValueIteration) {
      invokeFlashAttention_impl<T, SingleValueAttention>(
          batch_size_, head_num_, key_len_, seq_len_, size_per_head_, params,
          st);
    } else {
...
template <typename T>
FlashAttentionOp<T>::FlashAttentionOp(int batch_size, int head_num, int key_len,
                                      int seq_len, int size_per_head)
    : pimpl{std::make_unique<FlashAttentionOp<T>::impl>(
          batch_size, head_num, key_len, seq_len, size_per_head)} {}

template <typename T> FlashAttentionOp<T>::~FlashAttentionOp() {}

template <typename T> int FlashAttentionOp<T>::get_workspace_size() const {
