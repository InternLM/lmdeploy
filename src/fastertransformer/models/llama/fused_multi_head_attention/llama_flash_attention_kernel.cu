...
template<
    ...
    >
struct LlamaAttentionKernel: AttentionKernel<
  Element_,
  ArchTag,
  true,   // isAligned_
  kQueriesPerBlock,
  ...
  struct Params: BaseParams {
    ...
    CUTLASS_DEVICE void update_batched_ptr(ptr_t &data_ptr,
                                           ptr_t* batch_seq_ptr,
                                           int batch_seq_offset,
                                           int batch_id,
                                           int strideB) {
    ...
    CUTLASS_DEVICE bool advance_to_block() {
      ...
      output_ptr += int64_t(qo_start + query_start) * o_strideM() +
          head_id * o_strideH;

      if (output_accum_ptr != nullptr) {
        output_accum_ptr += int64_t(query_start) * o_strideM() +
            head_id * o_strideH;
      } else {
  ...
  static void CUTLASS_DEVICE attention_kernel(Params& p) {
    ...
    for (int32_t iter_key_start = 0; iter_key_start < p.num_keys;
      ...
      if (kSupportsBias && p.attn_bias_ptr != nullptr) {
        ...
        typename BiasLoader::SmemTileIterator smem_tile_iter(
...
private:
  ...
  using SingleValueAttention = LlamaAttentionKernel<
        ...
        true
    >;
  using MultiValueAttention = LlamaAttentionKernel<
        scalar_t,
        ArchTag,
        kQueriesPerBlock,
        kKeysPerBlock,
...
template<typename T>
FlashAttentionOp<T>::FlashAttentionOp(int batch_size,
      int head_num,
      int key_len,
      int seq_len,
      int size_per_head): 
...
template<typename T>
void FlashAttentionOp<T>::operator()(Params &params,
                                     cudaStream_t  st) const {
