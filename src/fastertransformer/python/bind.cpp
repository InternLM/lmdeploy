...
std::unique_ptr<DLManagedTensor> TritonTensorToDLManagedTensor(triton::Tensor& tensor) {
    ...
    DLTensor dl_tensor {
        .data=const_cast<void*>(tensor.data),
        .device=device,
    ...
    return std::unique_ptr<DLManagedTensor>(new DLManagedTensor{.dl_tensor=dl_tensor,
        .manager_ctx=nullptr,
        .deleter=[](DLManagedTensor*){}});
}
...
PYBIND11_MODULE(_turbomind, m) {
    ...
    py::class_<AbstractTransformerModel, std::shared_ptr<AbstractTransformerModel>>(m, "AbstractTransformerModel")
        ...
        .def("create_model_instance", [](
            ...
            "device_id"_a, "rank"_a, "stream"_a, "nccl_params"_a, "custom_all_reduce_comm"_a=nullptr)
        .def("create_shared_weights", &AbstractTransformerModel::createSharedWeights,
            "device_id"_a, "rank"_a)
