...
int MemoryPool::register_memory_region(std::string mr_key, uintptr_t data_ptr, uint64_t length)
{
    ...
    MIGRATION_ASSERT(mr, " Failed to register memory " << data_ptr);

    MIGRATION_LOG_INFO("Memory region: " << (void*)data_ptr << " -- " << (void*)(data_ptr + length)
                                         << ", Device name: " << pd_->context->device->dev_name
