...
{
    std::string lvl = get_env_variable("MIGRATION_LOG_LEVEL");
...
#define MIGRATION_ASSERT(Expr, Msg, ...)                                                                               \
    {                                                                                                                  \
        if (!(Expr)) {                                                                                                 \
            std::cerr << "\033[1;91m"                                                                                  \
                      << "[Assertion Failed]"                                                                          \
...
#define MIGRATION_ASSERT_EQ(A, B, Msg, ...) MIGRATION_ASSERT((A) == (B), Msg, __VA_ARGS__)
#define MIGRATION_ASSERT_NE(A, B, Msg, ...) MIGRATION_ASSERT((A) == (B), Msg, __VA_ARGS__)

#define MIGRATION_ABORT(Msg, ...)                                                                                      \
    {                                                                                                                  \
        std::cerr << ": \033[1;91m"                                                                                    \
                  << "[Fatal]"                                                                                         \
                  << "\033[m " << __FILE__ << ": " << __FUNCTION__ << ": Line" << __LINE__ << ": "                     \
...
#define MIGRATION_ERROR(Msg, ...) MIGRATION_ABORT(Msg, __VA_ARGS__)

#define MIGRATION_LOG_LEVEL(MsgType, Level, ...)                                                                       \
    {                                                                                                                  \
...
#define MIGRATION_LOG_INFO(...) MIGRATION_LOG_LEVEL("Info", 1, __VA_ARGS__)

#define MIGRATION_LOG_DEBUG(...) MIGRATION_LOG_LEVEL("Debug", 2, __VA_ARGS__)

#define MIGRATION_LOG_WARN(...) MIGRATION_LOG_LEVEL("Warn", 3, __VA_ARGS__)
