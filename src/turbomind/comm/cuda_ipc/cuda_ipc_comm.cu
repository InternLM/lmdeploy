...
CudaIpcCommImpl::~CudaIpcCommImpl()
{
    ...
    for (const auto& a : allocation_) {
        TM_LOG_WARN("[COMM][{}] Allocation ({}, {}) is not freed", global_rank_, a.uc_beg, a.size);
    }
...
void CudaIpcCommImpl::Free(void* ptr)
{
    if (auto it = allocation_.find(ptr); it != allocation_.end()) {
        ...
        TM_LOG_WARN("[TM][COMM][{}] Freeing {} which is not allocated by this module", global_rank_, ptr);
    }
}
...
void CudaIpcCommImpl::Register(void* ptr, size_t size)
{
    ...
    if (symm.find(ptr) != symm.end()) {
        TM_LOG_WARN("[TM][COMM][{}] Duplicated registration on ({}, {})", global_rank_, ptr, size);
        return;
    }
...
void CudaIpcCommImpl::Deregister(void* ptr)
{
    ...
    for (size_t i = 0; i < groups_.size(); ++i) {
        ...
        if (auto it = s.find(ptr); it != s.end()) {
            ...
            TM_LOG_WARN("[TM][COMM][{}] Deregistering non-registered address {}", global_rank_, ptr);
        }
    }
}
