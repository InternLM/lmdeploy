...
struct Config {
    ...
    using Impl   = Impl<MMA_SIMT, T, Tb, Tq, CTA_M, CTA_N, CTA_K, WARP_M, WARP_N, WARP_K, Stages, 1>;
    using Kernel = GemmUniversal<void, Mainloop_sm80<Impl>, CtaMap, AlignedM, AlignedN, SplitK>;
};

}  // namespace

void Registry::register_sm80_f16_f16()
{
    // clang-format off
    // Add(std::make_unique<KernelImpl<typename Config<128, 128, 32, 128, 16, 32, 3,  false, 1, 1>::Kernel>>());
    Add(std::make_unique<KernelImpl<typename Config<128, 128, 16, 64, 32, 16, 3,  true, 1, 1>::Kernel>>());
