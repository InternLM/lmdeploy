...
std::vector<LaunchSpec> StaticGemmContext::Populate(const Kernel& kernel, const PopulateParam& param) const
{
    ...
    for (int splits = 1; splits <= max_splits; ++splits) {
        ...
        const int64_t mio_cost_a = bytesize(desc.type_a, tiled_shape_n * m * split_ceil_k) * splits;
        const int64_t mio_cost_b = bytesize(desc.type_b, tiled_shape_m * n * split_ceil_k) * splits;
        /// TODO: read type from `desc_.accum` when added
        const int64_t mio_cost_c = bytesize(desc.type_c, (int64_t)m * n) * (splits - 1) * 2;
...
std::vector<LaunchSpec> MoeGemmContext::Populate(const Kernel& kernel, const PopulateParam& param) const
{
    ...
    for (int splits = 1; splits <= max_splits; ++splits) {
        ...
        // IO has less severe quantization effect
        const int64_t mio_cost_a = bytesize(desc.type_a, tiled_shape_n * batch_size * split_ceil_k) * num * splits;
        const int64_t mio_cost_b = bytesize(desc.type_b, tiled_shape_m * n * split_ceil_k) * num * splits;
        /// TODO: read type from `desc_.accum` when added
        const int64_t mio_cost_c = bytesize(desc.type_c, (int64_t)batch_size * n) * num * (splits - 1) * 2;
        const int64_t mio_cost   = mio_cost_a + mio_cost_b + mio_cost_c;
