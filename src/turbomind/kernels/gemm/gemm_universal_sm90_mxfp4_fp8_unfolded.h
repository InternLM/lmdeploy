...
struct GemmUniversalSm90MxFp4Fp8Unfolded {
    ...
    static_assert(kRestN >= 1 && kRestN <= 4,
                  "registered peak tiles use at most four residual-N atoms");
    ...
    using QparamRecordLayout = cute::Layout<
        cute::Shape<cute::Int<kOutputFragmentN>, cute::Int<kK32FragmentsPerStage>>,
        cute::Stride<cute::Int<kK32FragmentsPerStage>, cute::_1>>;
    using QparamSmemLogicalLayout = decltype(cute::tile_to_shape(
        QparamRecordLayout{},
    ...
    static_assert(cute::cosize_v<QparamSmemLogicalLayout> * sizeof(Tv)
                  == kMxScaleStageBytes * Stages);
    ...
    using SmemLayoutAtomD = decltype(
        gmma_ss_smem_selector<cute::GMMA::Major::MN,
                              cutlass::bfloat16_t,
                              cute::Int<kEpiN>,
                              cute::Int<kEpiM>>());
    using SmemLayoutD = decltype(cute::tile_to_shape(
        SmemLayoutAtomD{},
        cute::make_shape(cute::Int<kEpiN>{},
                         cute::Int<kEpiM>{},
                         cute::Int<kEpiPlanes>{}),
    ...
    using QparamCtaTile = cute::Shape<cute::Int<kQparamTmaInnerValues>,
                                      cute::Int<kQparamTmaParts * kOutputFragments>,
                                      cute::_1>;
    using QparamSmemLayout = decltype(cute::make_layout(QparamCtaTile{}));
    using QparamPipelineLayout = cute::Layout<
        cute::Shape<cute::Int<cute::cosize_v<QparamSmemLayout>>, cute::Int<Stages>>>;
    ...
    static auto MakeTmaActivation(void* ptr, int m, int k, int ld)
    {
        auto gmem = cute::make_tensor(
            cute::make_gmem_ptr(reinterpret_cast<Ta*>(ptr)),
    ...
    static auto MakeTmaQparam(void* ptr, int n, int k)
    {
        ...
        auto layout = cute::make_layout(
            ...
            cute::make_stride(cute::_1{},
                              cute::Int<kQparamTmaInnerValues>{},
                              int64_t{kQparamValuesPerFragment} * out_fragments));
    ...
    static_assert(cute::size(typename TmaQparam::Traits::SrcLayout{})
                  == kMxScaleStageBytes * 8);
                               ...
                               char* smem_buf)
    {
        ...
        params.transaction_bytes = kPackedWeightStageBytes + kMxScaleStageBytes
...
private:
                                            ...
                                            MainloopPipeline& pipeline)
    {
        ...
        if (warp_in_wg == 0) {
            ...
            auto sQparamPipeline = cute::make_tensor(
                cute::make_smem_ptr(storage.V.data()), QparamPipelineLayout{});
            auto sActivation = cute::make_tensor(
                cute::make_smem_ptr(storage.B.data()), SmemLayoutB{});
            auto activation_gmem = tm_a.get_tma_tensor(
                cute::make_shape(sched.gemm_shape().x, k_iters * TILE_K));
                ...
                if (tile->is_valid_cluster && elected) {
                    ...
                    for (int k_tile = 0; k_tile < k_iters; ++k_tile) {
                        ...
                        auto activation_tile = cute::local_tile(
                            activation_gmem,
                            ActivationCtaTile{},
                            cute::make_coord(tile->offset_m / TILE_M, k_tile));
                        ...
                        cute::copy(
                            tm_a.with(Adesc, *bar, 0, cute::TMA::CacheHintSm90::EVICT_LAST),
                            activation_src,
                            activation_dst);
                                        ...
                                        MainloopPipeline& pipeline)
    {
        ...
        auto cC = cute::make_identity_tensor(
            cute::Shape<cute::Int<kComputeTileN>, cute::Int<TILE_M>>{});
        auto tCcC = thr_mma.partition_C(cC);
        auto cA = cute::make_identity_tensor(
            cute::Shape<cute::Int<kComputeTileN>, cute::Int<TILE_K>>{});
        auto tAcA = thr_mma.partition_A(cA);
        auto dummy_sA = cute::make_tensor(
            cute::make_smem_ptr(reinterpret_cast<ElementMmaA*>(storage.A.data())),
            typename Traits::DummySmemLayoutA{});
        auto dummy_tCsA = thr_mma.partition_A(dummy_sA);
        auto tCrA = thr_mma.make_fragment_A(
            dummy_tCsA(cute::_, cute::_, cute::_, cute::Int<0>{}));
        auto sB = cute::make_tensor(cute::make_smem_ptr(storage.B.data()), SmemLayoutB{});
        auto sScale = cute::make_tensor(
            cute::make_smem_ptr(storage.U.data()),
            cute::make_layout(cute::Shape<cute::Int<kUStageStride>, cute::Int<Stages>>{}));
        auto warp_group_thread_layout = cute::make_layout(
            cute::Int<kMathWarpGroups>{}, cute::Int<WARPGROUP_SIZE>{});
        ...
        constexpr int kPackedWordsPerThread =
            cute::size<1>(decltype(layout_a_tv){})
            * cute::sizeof_bits_v<cute::uint4_t> / cute::sizeof_bits_v<uint32_t>;
        static_assert(kPackedWordsPerThread == 2);
        auto packed_copy = cute::make_tiled_copy(
            cute::Copy_Atom<cute::UniversalCopy<uint32_t>, uint32_t>{},
        ...
        auto sQparam = cute::make_tensor(
            cute::make_smem_ptr(storage.V.data()),
            QparamSmemLogicalLayout{});
        ...
        auto run = [&] {
            ...
            while (tile->alive) {
                if (tile->is_valid_cta) {
                    ...
                    for (int k_tile = 0; k_tile < sched.k_iters_; ++k_tile) {
                        ...
                        auto packed_stage = sPackedPipeline(cute::_, stage);
                        auto sPacked = cute::make_tensor(
                            packed_stage.data(), PackedSmemLayout{});
                        ...
                        auto sQparamStage = sQparam(cute::_, cute::_, stage);
                        {
                            ...
                            auto scratch = cute::make_fragment_like(
                                accum(cute::_, cute::Int<0>{}, cute::Int<0>{}));
                            ...
                            constexpr bool kPreloadActivationScales =
                                kOpN == 128 && kRestM == 1 && kRestN == 1;
                            auto activation_scales = [&] {
                                if constexpr (kPreloadActivationScales) {
                                    return cute::make_tensor<float>(cute::make_layout(cute::Shape<
                                        cute::Int<kCValue0>, cute::Int<kCValue2>, cute::Int<kRestN>>{}));
                                }
                                else {
                                ...
                                cute::for_each(cute::make_seq<kRestN>{}, [&](auto rest_n) {
                                    cute::for_each(cute::make_seq<kCValue2>{}, [&](auto v2) {
                                        cute::for_each(cute::make_seq<kCValue0>{}, [&](auto v0) {
                                            const auto coord = tCcC(
                                                cute::make_coord(v0, cute::Int<0>{}, v2),
                                                cute::Int<0>{},
                                                rest_n);
                            ...
                            cute::for_each(cute::make_seq<kRestM>{}, [&](auto rest_m) {
                                ...
                                for (int kb = 0; kb < kQparamGroups; ++kb) {
                                    ...
                                    detail::unpack_e2m1x8_to_e4m3x4x2(
                                        ...
                                        a_words(cute::make_coord(cute::make_coord(
                                    ...
                                    detail::unpack_e2m1x8_to_e4m3x4x2(
                                        ...
                                        a_words(cute::make_coord(cute::make_coord(
                                            cute::Int<0>{}, cute::Int<0>{}, cute::Int<1>{}))),
                                        a_words(cute::make_coord(cute::make_coord(
                                            cute::Int<0>{}, cute::Int<1>{}, cute::Int<1>{}))));
                                }
                            });
                            ...
                            auto issue = [&](auto kb, auto rest_m, auto rest_n, auto& frag) {
                                ...
                                cute::gemm(tiled_mma,
                                           tCrA(cute::_, rest_m, kb),
                                           tCrB(cute::_, rest_n, kb, stage),
                                           frag);
                            ...
                            auto scale = [&](auto kb, auto rest_m, auto rest_n, auto& src) {
                                ...
                                const auto weight_coord0 = tCcC(
                                    cute::make_coord(cute::Int<0>{}, cute::Int<0>{}, cute::Int<0>{}),
                                    rest_m,
                                    cute::Int<0>{});
                                const auto weight_coord1 = tCcC(
                                    cute::make_coord(cute::Int<0>{}, cute::Int<1>{}, cute::Int<0>{}),
                                    rest_m,
                                    cute::Int<0>{});
                                const uint8_t weight_exponent0 =
                                    sQparamStage(cute::get<0>(weight_coord0), kb);
                                const uint8_t weight_exponent1 =
                                    sQparamStage(cute::get<0>(weight_coord1), kb);
                                if constexpr (kPreloadActivationScales) {
                                    CUTE_UNROLL
                    ...
                    {
                        nv_bfloat16* smem_C = reinterpret_cast<nv_bfloat16*>(
                            reinterpret_cast<char*>(&storage) + kOutputOffset);
                        auto sD = cute::as_position_independent_swizzle_tensor(cute::make_tensor(
                            cute::make_smem_ptr(smem_C), SmemLayoutD{}));
                        using EpiTiledMma =
                            std::conditional_t<kSplitEpiM, WgTiledMma, TiledMma>;
                        EpiTiledMma epi_tiled_mma;
                        auto tiled_copy_c =
                            cute::make_tiled_copy_C_atom(CopyAtomC{}, epi_tiled_mma);
                        auto tiled_r2s = cute::make_tiled_copy_S(
                            cute::Copy_Atom<CopyOpR2S, cutlass::bfloat16_t>{}, tiled_copy_c);
                        ...
                        auto tRS_rD = cute::make_tensor<cutlass::bfloat16_t>(
                            cute::make_layout(cute::take<0, 3>(
                                cute::shape(thr_r2s.partition_S(sD)))));
                        auto tRS_rAcc_frg = cute::recast<cutlass::Array<float, kFragmentSize>>(tRS_rAcc);
                        auto tRS_rD_frg =
                            cute::recast<cutlass::Array<cutlass::bfloat16_t, kFragmentSize>>(tRS_rD);
                        constexpr int kMmaTileN =
                            cute::size<0>(typename Traits::TileShape{}) / cute::size<1>(decltype(tRS_rAcc){});
                        constexpr int kMmaTileM =
                            (kSplitEpiM ? kWgM : cute::size<1>(typename Traits::TileShape{}))
                            / cute::size<2>(decltype(tRS_rAcc){});
                        static_assert(kMmaTileN == kEpiN);
                        static_assert(kMmaTileM % kEpiM == 0);
                        auto r2s_value_layout = cute::make_layout(
                            cute::make_shape(cute::size(tRS_rD_frg),
                                             cute::Int<kMmaTileM / kEpiM>{}));
                        auto epi_pass_layout = cute::make_layout(
                            cute::make_shape(cute::Int<kEpiStripsM>{},
                                             cute::Int<kEpiStripsN>{}),
                            cute::make_stride(cute::Int<kEpiStripsN>{}, cute::_1{}));
                        auto store_offset_m_layout = cute::make_layout(
                            cute::make_shape(cute::Int<kEpiPlanes>{},
                                             cute::Int<kEpiStripsM>{},
                                             cute::_1{}),
                            cute::make_stride(cute::Int<kWgM>{},
                                              cute::Int<kEpiM>{},
                                              cute::Int<kTmaStoreM>{}));
                        auto store_offset_n_layout = cute::make_layout(
                            cute::make_shape(cute::Int<kEpiStripsN>{},
                                             cute::Int<kTmaStoreCountN>{}),
                            cute::make_stride(cute::Int<kEpiN>{}, cute::Int<kTmaStoreN>{}));
                        auto store_tile_layout = cute::make_layout(
                            cute::make_shape(cute::Int<kTmaStoreCountN>{},
                                             cute::Int<kEpiPlanes>{}));
                        ...
                        auto epi_synchronize = [&] {
                            if constexpr (kSplitEpiM) {
                                ...
                                named_barrier_arrive_and_wait(
                                    kMathThreads, kEpilogueBarrierId);
                            }
                        };
                        ...
                        for (int epi_m = 0; epi_m < kEpiStripsM; ++epi_m) {
                            ...
                            for (int epi_n = 0; epi_n < kEpiStripsN; ++epi_n) {
                                ...
                                if (tma_store_leader) {
                                    ...
                                    const int tma_m =
                                        (warp_in_wg + kTmaStoreWarpsPerWg - first_warp)
                                        % kTmaStoreWarpsPerWg;
                                    if (tma_m == 0) {
                                        const auto store_tile_coord =
                                            store_tile_layout.get_flat_coord(store_wg);
                                        const int tma_n = cute::get<0>(store_tile_coord);
                                        const int store_plane = cute::get<1>(store_tile_coord);
                                        const int store_m =
                                            tile->offset_m
                                            + store_offset_m_layout(store_plane, epi_m, 0);
                                        const int store_n =
                                            tile->offset_n + store_offset_n_layout(epi_n, tma_n);
                                        auto sD_tma = cute::local_tile(
                                            sD_epi,
                                            cute::make_shape(cute::Int<kTmaStoreN>{},
                                                             cute::Int<kTmaStoreM>{}),
                                            cute::make_coord(tma_n, 0));
                                        cute::SM90_TMA_STORE::copy(
                                            &tm_c,
                                            cute::raw_pointer_cast(sD_tma.data()),
                                            store_n,
                                            store_m);
                                    }
            ...
            cutlass::arch::NamedBarrier::sync(
                kMathThreads, cutlass::arch::ReservedNamedBarriers::EpilogueBarrier);
        };
