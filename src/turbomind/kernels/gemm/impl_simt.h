...
struct Impl<MMA_SIMT, T_, Tb_, Tq_, CTA_M_, CTA_N_, CTA_K_, WARP_M_, WARP_N_, WARP_K_, Stages_, Flag_> {
    ...
    // (128, 128, 16), (128, 16, 16), ( 2, 16,  8)

    // (  1, 128, 16), (  1, 16, 16), ( 1, 16, 16) -> 1 x (k8,  m1) (k8, n16) -> TN
    ...
    // using SmemLayoutB = SmemLayoutV2<CTA_K, CTA_N>;
    // using ThreadMapB  = gemm::ThreadMap<CTA_N, CTA_K, 8, WARP_CNT>;
    ...
    struct StateB {
        ...
        __device__ void Load(int k, int)
        {
            ...
            // const int offset_s = warp_id_k(warp_id) * WARP_K;
            // const int offset_c = warp_id_n(warp_id) * WARP_N;
            ...
            //     const int s = offset_s + thr_s + k * OP_K;
            //     const int c = offset_c + thr_c + n * OP_N;
            //     Lds(frag_B[k][n], &smem_B(s, c));
            // }
    ...
    __device__ static void StoreC(FragC& frag_C, SharedStorage& storage, Func&& func)
    {
        ...
        for (int m = 0; m < ITER_M; ++m) {
            ...
            for (int n = 0; n < ITER_N; ++n) {
                ...
                //     auto&     tmp = (Array<float, 4>&)frag_C[m][n][nn];
                //     const int mi  = m * OP_M + lane_id % 16;
                //     const int ni  = n * OP_N + lane_id / 16 * 8 + nn;
