...
CUTE_HOST_DEVICE constexpr uint32_t encode_e2m1x4_lanes_unscaled(uint32_t lane)
{
    ...
    const uint32_t     mantissa    = ((lane & kLaneBit0) & ((lane >> 1) | (lane >> 2)) & kLaneBit0) << 2;
    return sign | e2 | e1 | (low_nonzero << 4) | (low_nonzero << 5) | mantissa;
}

__device__ __forceinline__ void unpack_e2m1x8_to_e4m3x4x2(uint32_t packed, uint32_t& row_lo, uint32_t& row_hi)
{
...
CUTE_HOST_DEVICE constexpr E2m1E4m3ScaleTable make_e2m1_e4m3_scale_table(uint32_t shift)
{
...
decode_e2m1x8_paired_signs(uint32_t packed, const E2m1E4m3ScaleTables& tables, uint32_t& row_lo, uint32_t& row_hi)
{
    ...
    const uint32_t lo_value    = prmt(tables.values0_to_3_lo, tables.values4_to_7_lo, lo_selector);
    const uint32_t hi_value    = prmt(tables.values0_to_3_hi, tables.values4_to_7_hi, hi_selector);
    row_lo                     = lo_value | (packed & kE4m3Signs);
    row_hi                     = hi_value | ((packed << 4) & kE4m3Signs);
}

__device__ __forceinline__ void
unpack_e2m1x16_to_e4m3x4x4(const uint32_t* packed, const E2m1E4m3ScaleTables& tables, uint32_t* out)
{
