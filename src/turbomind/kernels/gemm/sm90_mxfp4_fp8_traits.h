...
struct GmmaMxFp4Fp8TraitsBase {
    ...
    using MmaAtom     = decltype(
        cute::GMMA::
            rs_op_selector<ElementA, ElementB, ElementC, WgTileShape, cute::GMMA::Major::K, cute::GMMA::Major::K>());
    ...
    CUTE_HOST_DEVICE static constexpr auto packed_layout_a_mk()
    {
        ...
        auto packed_value        = cute::make_layout(value_shape,
                                              cute::make_stride(cute::make_stride(cute::_2{}, cute::_1{}, cute::_8{}),
                                                                cute::make_stride(cute::_0{}, cute::_0{})));
        auto packed_value_linear = cute::composition(packed_value, cute::right_inverse(cute::make_layout(value_shape)));
        auto packed_tv = cute::make_layout(cute::make_layout(cute::size<0>(layout_a_tv), cute::size<1>(layout_a_tv)),
                                           packed_value_linear);
    ...
    CUTE_HOST_DEVICE static constexpr auto folded_layout_a_mk()
    {
        ...
        auto       layout_a_tv = mma.get_layoutA_TV();
        auto       layout_a_mk = cute::get<0>(mma.get_layoutA_MK());
        auto       thread_major_tv =
            cute::make_layout(cute::make_shape(cute::size<0>(layout_a_tv), cute::size<1>(layout_a_tv)),
    ...
    using SmemLayoutAtomB =
        decltype(gmma_ss_smem_selector<cute::GMMA::Major::K, ElementB, cute::Int<TILE_BATCH>, cute::Int<TILE_K>>());
...
struct GmmaMxFp4Fp8UnfoldedTraits: GmmaMxFp4Fp8TraitsBase<Out, Batch, Stages, WGLayout, MmaN> {
    ...
    static constexpr int kActivationScaleRegs = Base::kRestN > 1 ? 2 : Base::kActScalesPerAtom* Base::kRestN;
    static constexpr int kExplicitRegs = kARegs + kAccumRegs + kScratchRegs + kQparamRegs + kActivationScaleRegs + 2;
