...
bool MatchesGdrSpec(const GdrKernelSpec& spec,
                    const Operation& operation,
                    const PlanningContext& context)
{
    const int requested_chunk = operation.mode == GdrMode::kRecurrent
                                    ? kRecurrentGdrChunkSize
    ...
    return operation.mode == spec.mode && context.input_dtype == spec.input_dtype
           ...
           && (operation.mode != GdrMode::kRecurrent
               || operation.chunk_size == kAutoGdrChunkSize
...
void BuildOptimizedTensorPlans(Plan* plan, size_t direct_descriptor_bytes)
{
    ...
    plan->out = TensorPlan{core::Layout{{problem.batch, problem.token_num, problem.hv, 128},
                                        {value_batch, value_row, 128, 1}},
                           problem.input_dtype,
                           value_elements};
    ...
    plan->resolvent = TensorPlan{core::Layout{{problem.batch,
                                               problem.token_num,
                                               problem.hv,
                                               problem.chunk_size},
                                              {resolvent_batch,
    ...
    if (IsChunkedGdr(problem)) {
        ...
        AddWorkspaceBytes(&plan->workspace_bytes,
                          resolvent_elements * byte_size(problem.input_dtype, 1));
        AddWorkspaceBytes(&plan->workspace_bytes,
                          plan->cp.enabled ? plan->cp.workspace_bytes : direct_descriptor_bytes,
                          plan->cp.enabled ? 16 : 128);
    }
    plan->workspace = TensorPlan{core::Layout{{core::ssize_t(plan->workspace_bytes)}},
                                 kUint8,
