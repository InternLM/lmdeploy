...
__global__ __launch_bounds__(kFusedGdrThreads, 1) void
Sm120FusedGdrFwdKernel(const CUtensorMap* __restrict__ tma_desc_workspace,
                                  ...
                                  int hv)
{
    ...
    if (wg_idx == 3) {
        ...
        constexpr int kEarlyTmaBytesPerRow = BlockDv * static_cast<int>(sizeof(T))
                                             + kChunk32Size * static_cast<int>(sizeof(T));
        ...
        if (q_leader) {
            for (int load_chunk = 0; load_chunk < chunks; ++load_chunk) {
                ...
                const int token0   = token_base + load_chunk * kChunk32Size;
                cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(
                    &q_ready_mbar, kTmaBoxRows * kQkTmaBytesPerRow);
        ...
        if (k_leader) {
            for (int load_chunk = 0; load_chunk < chunks; ++load_chunk) {
                ...
                const int token0   = token_base + load_chunk * kChunk32Size;
                cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(
                    &k_ready_mbar, kTmaBoxRows * kQkTmaBytesPerRow);
        ...
        if (store_leader) {
            for (int chunk = 0; chunk < chunks; ++chunk) {
                ...
                cute::SM90_TMA_STORE_4D::copy(
                    out_desc, out_stage, dv0, value_head, token0, 0);
...
#pragma unroll
            ...
            auto s_k_t = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.k_stage[data_buf][0][0])),
            ...
            if (chunk == chunks - 1 && !context_parallel) {
                ...
                if constexpr (std::is_same_v<StateT, float>) {
                    ...
                    FusedGdrStoreStateFragmentBf16Tma<BlockDv>(
                        tCrState,
                        tCcState,
                        reinterpret_cast<__nv_bfloat16*>(&smem.state_stage[0][0]));
                }
                FusedGdrMmaSyncNamed<kFusedGdrBarrierStateUpdate>();
            }
            if (role_tid == 0) {
        ...
        for (int chunk = 0; chunk < chunks; ++chunk) {
            ...
            auto s_k_smem = cute::make_tensor(
                cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.k_stage[data_buf][0][0])),
                Sm120FusedGdrQkLayout());
            auto s_a_smem = cute::make_tensor(
                cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.p_stage[data_buf][0][0])),
            ...
            auto s_state = cute::make_tensor(
                cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.vd[0][0])),
            ...
            auto s_w = cute::make_tensor(
                cute::make_smem_ptr(w_pack),
                Sm120FusedGdrVTLayout<BlockDv>());
            cute::clear(tCrC);
            cute::cooperative_gemm(role_tid,
                                   mma,
        ...
        for (int chunk = 0; chunk < chunks; ++chunk) {
            ...
            auto s_q_smem = cute::make_tensor(
                cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.q_stage[data_buf][0][0])),
                Sm120FusedGdrQkLayout());
            auto s_k_smem = cute::make_tensor(
                cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.k_stage[data_buf][0][0])),
                Sm120FusedGdrQkLayout());
            ...
            for (int i = 0; i < cute::size(tCrC); i += 2) {
                ...
                const float p0 = row < valid && col < valid && col <= row
                                     ? kHeadScale * tCrGRel(i) * tCrC(i)
                                     : 0.0f;
            ...
            auto s_packed_vd = cute::make_tensor(
                cute::make_smem_ptr(packed_vd),
                Sm120FusedGdrVTLayout<BlockDv>());
            auto* out_stage = reinterpret_cast<Element*>(&smem.q_stage[data_buf][0][0]);
            auto s_out = cute::make_tensor(
                cute::make_smem_ptr(out_stage),
                Sm120FusedGdrVRowLayout<BlockDv>());
...
void SetFusedGdrFwdSharedMemoryLimit(size_t smem_bytes)
{
    ...
    static const cudaError_t status =
        cudaFuncSetAttribute(Sm120FusedGdrFwdKernel<__nv_bfloat16, StateT, BlockDv>,
