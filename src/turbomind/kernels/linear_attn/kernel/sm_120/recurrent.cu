...
constexpr int kRecurrentMaxDynamicSharedBytes =
...
RecurrentUnpackStateTma(float (&state_stage)[kHeadDim][BlockDv], int tid, int threads)
{
...
template<class StateT, int BlockDv>
__device__ __forceinline__ void
RecurrentPackStateTma(float (&state_stage)[kHeadDim][BlockDv], int tid, int threads)
{
    if constexpr (!std::is_same_v<StateT, float>) {
        static_assert(std::is_same_v<StateT, __nv_bfloat16>);
        static_assert(BlockDv % 2 == 0);
                                   ...
                                   int                                 state_layer)
{
    ...
    const int64_t value_offset = static_cast<int64_t>(batch) * out_batch_stride
    ...
    if (loader_tid == 0) {
            ...
            cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(
                &smem.state_tma_ready[stage], RecurrentStateTmaBytes<StateT, BlockDv>());
            if (hv >= 32) {
...
template<int BlockDv, class StateT>
__device__ void ComputeRecurrentTile(Sm120GdrRecurrentSharedStorage<BlockDv>& smem,
                                     int                                 stage,
                                     int                                 role_tid)
{
...
template<int BlockDv, class StateT>
__global__ __launch_bounds__(kRecurrentThreads, 2) void Sm120GdrRecurrentKernel(
    ...
    int                  state_layer)
{
    ...
    auto& smem = *reinterpret_cast<Sm120GdrRecurrentSharedStorage<BlockDv>*>(smem_raw);
    ...
    if (wg_idx == kRecurrentConsumerWarpGroups) {
        ...
        if (role_tid < kRecurrentStoreThreads) {
            ...
            for (int work_tile = first_work_tile; work_tile < total_work_tiles;
                ...
                if (role_tid == 0 && !tile_finished) {
                    ...
                    cute::SM90_TMA_STORE_4D::copy(
                        &state_tma_descs[state_desc_index],
                        reinterpret_cast<StateT*>(&smem.state[stage][0][0]),
                        dv0,
                        0,
                        local_head,
            ...
            for (int work_tile = first_work_tile; work_tile < total_work_tiles;
                ...
                StageRecurrentTile<BlockDv, StateT>(
                    smem,
                    stage,
                    head_tile,
                    dv_tile,
                                ...
                                cudaStream_t        stream)
{
    ...
    const int state_layer =
        static_cast<int>(state_layer_offset
                         / (static_cast<int64_t>(problem.heads_per_block) * kHeadDim * kHeadDim));
    ...
    const bool medium_persistent_grid =
        total_work_tiles > base_grid_blocks * kRecurrentMediumMinWorkPerBaseCta
    ...
    const size_t smem_bytes = Sm120GdrRecurrentSharedBytes<block_dv>();
    ...
    SetRecurrentGdrSharedMemoryLimit<block_dv, StateT>(smem_bytes);
    Sm120GdrRecurrentKernel<block_dv, StateT><<<grid_blocks, kRecurrentThreads, smem_bytes, stream>>>(
        q_tma_desc,
        k_tma_desc,
        v_tma_desc,
        out_ptr,
                          ...
                          cudaStream_t stream)
{
    if (state_dtype == kFloat32) {
        LaunchSm120GdrRecurrentTyped<float>(q,
                                         k,
                                         v,
                                         g,
                                         beta,
        ...
        LaunchSm120GdrRecurrentTyped<__nv_bfloat16>(q,
                                                 k,
                                                 v,
                                                 g,
                                                 beta,
