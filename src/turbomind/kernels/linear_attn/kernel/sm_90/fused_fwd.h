// Inspired by https://github.com/QwenLM/FlashQLA/blob/60f81453143e724bcaf3fc7921e71e7328f6ebcd/flash_qla/ops/gated_delta_rule/chunk/hopper/fused_fwd.py
...
{
    return problem.arch == 900 && problem.input_dtype == kBfloat16
           && problem.batch == problem.sequence_num && problem.gate_stride % 4 == 0
           && problem.gate_batch_stride % 4 == 0 && problem.hv % problem.hq == 0 && problem.head_dim == kHeadDim
           && problem.chunk_size == kChunkSize;
}
...
static __device__ __forceinline__ void StorePackedBf16Stsm(Fragment const&   fragment,
...
template<class TiledMma>
using GmmaCThreadLayout = decltype(cute::make_layout(
    cute::get<0>(cute::shape(GmmaCLayout<TiledMma>{})),
...
static CUTE_HOST_DEVICE constexpr auto GmmaCIdentityTensor()
{
    ...
    static_assert(decltype(cute::size(CLayout{}))::value
                  == decltype(cute::size<0>(AtomShape{}))::value
                         * decltype(cute::size<1>(AtomShape{}))::value);

    return cute::make_identity_tensor(
        cute::make_shape(cute::get<0>(AtomShape{}), cute::get<1>(AtomShape{})));
}
...
static CUTE_HOST_DEVICE constexpr auto GmmaCFragmentCoord(Pair pair, RowSelect row_select, Stripe stripe)
{
    return cute::make_coord(
        cute::make_coord(pair, row_select, stripe), cute::Int<0>{}, cute::Int<0>{});
}

template<class TiledMma, class RowMap, class ColMap, class Body>
static __device__ __forceinline__ void ForEachProjectedGmmaC(GmmaCThreadProjection projection,
                                                             RowMap               row_map,
                                                             ColMap               col_map,
                                                             Body                 body)
{
...
template<class Element,
         ...
         class RsFragment>
static __device__ __forceinline__ void PackSquareCAsRsA(SquareFragment const& tCrP,
                                                         RsFragment&            tCrPrs)
{
    ...
    cute::for_each(cute::make_seq<8>{}, [&](auto stripe) {
        ...
        cute::for_each(cute::make_seq<2>{}, [&](auto row_select) {
            cute::for_each(cute::make_seq<2>{}, [&](auto pair) {
                ...
                auto a_coord = cute::make_coord(cute::make_coord(pair,
                                                                  row_select,
                                                                  cute::Int<innerK>{}),
                                                 cute::Int<0>{},
                                                 cute::Int<kBlock>{});
                tCrPrs(a_coord) =
                    Element(CastFromFloat<T>(static_cast<float>(tCrP(c_coord))));
            });
        });
    });
}
                                                                  ...
                                                                  int64_t gate_stride)
    {
        ...
        for (int row = lane; row < kChunkSize; row += kCudaWarpThreads) {
            ...
            cute::SM80_CP_ASYNC_CACHEALWAYS_ZFILL<float>::copy(
                gate[source_offset], gate_stage[row], in_bounds);
        }
    }
    ...
    const int64_t gate_sequence_offset =
        static_cast<int64_t>(gate_physical_batch) * gate_batch_stride
        + static_cast<int64_t>(gate_local_sequence_begin) * gate_stride + value_head;
            ...
            for (int chunk = 0; chunk < chunks; ++chunk) {
                ...
                if (role_tid == 0) {
                    ...
                    cutlass::arch::ClusterTransactionBarrier::expect_transaction(
                        &smem.stage_ready_mbar[stage], 2 * kQkTmaBytes);
        ...
        if (role_tid < 64) {
            for (int chunk = 0; chunk < chunks; ++chunk) {
                ...
                if (role_tid == 32) {
                    ...
                    cutlass::arch::ClusterTransactionBarrier::expect_transaction(
                        &smem.stage_ready_mbar[stage], kValueTmaBytes);
        ...
        if (role_tid < 96) {
            for (int chunk = 0; chunk < chunks; ++chunk) {
                ...
                if (role_tid == 64) {
                    ...
                    cutlass::arch::ClusterTransactionBarrier::expect_transaction(
                        &smem.stage_ready_mbar[stage], kSquareTmaBytes);
            ...
            if (output_store_lane == 0) {
                ...
                if constexpr (BlockDv == kWideGdrBlockDv) {
                    cute::SM90_TMA_STORE_4D::copy(out_desc,
                                                  &smem.o_shared[output_stage][0][0],
                                                  dv0,
                                                  value_head,
                                                  token0,
                                                  0);
    ...
    if (wg_idx == 0) {
        ...
        for (int chunk = 0; chunk < chunks; ++chunk) {
            ...
            auto s_k_t = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.k_stage[stage][0][0])),
        ...
        auto s_sv_c = cute::make_tensor(
            cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.o_shared[0][0][0])),
        ...
        for (int chunk = 0; chunk < chunks; ++chunk) {
            ...
            if (gate_warp_tid < kFusedGdrGateRowsPerWarp) {
                ...
                for (int pass = 0; pass < kFusedGdrGatePasses; ++pass) {
                    const int row = pass * kFusedGdrGateWriterThreads
                                    + gate_warp * kFusedGdrGateRowsPerWarp
                                    + gate_warp_tid;
            ...
            auto s_k = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.k_stage[stage][0][0])),
            ...
            auto s_v = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.v_stage[stage][0][0])),
                                         FusedGdrSwizzledVTLayout<BlockDv>());
            auto s_w = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.v_stage[stage][0][0])),
                                         FusedGdrGmmaVdTLayout<Element, BlockDv>());
            ForEachProjectedGmmaC<ValueMma>(
                c_projection,
                [&](int row) {
                    return smem.g_exp[row];
                },
                [&](auto fragment_coord, int row, int dv, float row_g_exp) {
                    float w = 0.0f;
                    if (row < valid) {
                        w = static_cast<float>(s_v(dv, row))
                            - row_g_exp * static_cast<float>(tCrU(fragment_coord));
                    }
            ...
            FusedGdrMmaSyncNamed<kFusedGdrBarrierValueU>();
            auto s_w_store = cute::make_tensor(
                cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.v_stage[stage][0][0])),
            ...
            auto s_a = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.a_stage[stage][0][0])),
            ...
            ForEachProjectedGmmaC<ValueMma>(
                ...
                [&](auto fragment_coord, int row, int, int) {
                    tCrU(fragment_coord) =
                        row < valid ? static_cast<float>(tCrU(fragment_coord)) : 0.0f;
                });
            auto s_vd_store = cute::make_tensor(
                cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.vd_shared[0][0])),
                FusedGdrGmmaVdRowLayout<Element, BlockDv>());
            ...
            ForEachProjectedGmmaC<ValueMma>(
                c_projection,
                [&](int row) {
                    return smem.g_rev_exp[row];
                },
                [&](auto fragment_coord, int, int, float row_g_rev_exp) {
                    tCrU(fragment_coord) =
                        row_g_rev_exp * static_cast<float>(tCrU(fragment_coord));
                });
            auto s_vn_store = cute::make_tensor(
        ...
        for (int chunk = 0; chunk < chunks; ++chunk) {
            ...
            auto s_q = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.q_stage[stage][0][0])),
                                         FusedGdrGmmaQkKLayout<Element>());
            auto s_k = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.k_stage[stage][0][0])),
                                         FusedGdrGmmaQkKLayout<Element>());
            auto s_ag = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.a_stage[stage][0][0])),
                                          FusedGdrGmmaSquareKLayout<Element>());
            ...
            cute::wait_barrier(smem.gate_ready_bar, phase);
            auto s_a_raw = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.a_stage[stage][0][0])),
                                             FusedGdrSwizzledA64Layout());
            {
                ...
                ForEachProjectedGmmaC<SquareMma>(
                    ...
                    [&](int column) {
                        const float beta_column =
                            column < valid ? smem.gate_stage[stage][1][column] : 0.0f;
                    ...
                    [&](auto fragment_coord, int row, int column, float row_g, auto column_data) {
                        ...
                        const float a_value =
                            static_cast<float>(s_a_raw(row, column)) * g_rel * beta_column;
                        const float p_value =
                            static_cast<float>(tCrP(fragment_coord)) * kHeadScale * g_rel;
            ...
            ForEachProjectedGmmaC<OutputMma>(
                ...
                [&](auto fragment_coord, int row, int, float row_g_exp) {
                    if (row < valid) {
                        tCrO(fragment_coord) =
                            static_cast<float>(tCrO(fragment_coord)) * kHeadScale * row_g_exp;
                    }
                    else {
