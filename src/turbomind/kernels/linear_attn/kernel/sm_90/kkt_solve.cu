...
static CUTE_HOST_DEVICE constexpr auto BetaStageLayout()
{
    return cute::composition(
        cute::Swizzle<3, 2, 3>{},
        cute::Layout<cute::Shape<cute::_64, cute::_4>,
...
static CUTE_HOST_DEVICE constexpr auto Block16Layout()
{
    return cute::composition(
        cute::Swizzle<2, 2, 3>{},
        cute::make_layout(cute::make_shape(cute::Int<Tiles>{},
                                           cute::Int<kBlock16Stride>{},
                                           cute::Int<kBlock16>{}),
    ...
    return cute::composition(
        ...
        cute::Layout<cute::Shape<cute::Shape<cute::Int<kKTileSwizzleDim>, cute::_2>,
                     ...
                     cute::Stride<cute::Stride<cute::_1, cute::Int<kKTilePlaneElems>>,
                                  cute::Stride<cute::Int<kKTileSwizzleDim>,
                                               cute::Int<kKTileSwizzleDim * 8>>>>{});
}
...
static CUTE_HOST_DEVICE constexpr auto RepackUpperVec4ThreadLayout()
{
    return cute::make_layout(
        cute::make_shape(cute::Int<2>{},
                         cute::make_shape(cute::Int<4>{}, cute::Int<4>{}),
                         cute::Int<kHalfChunk / kVec4Elems>{}),
        cute::make_stride(cute::Int<128>{},
                          cute::make_stride(cute::Int<32>{}, cute::Int<8>{}),
                          cute::Int<1>{}));
}
...
{
    return cute::make_layout(
        cute::make_shape(cute::make_shape(cute::Int<8>{}, cute::Int<4>{}),
                         cute::Int<kHalfChunk / kVec8Elems>{}),
        cute::make_stride(cute::make_stride(cute::Int<1>{}, cute::Int<32>{}),
                          cute::Int<8>{}));
}
...
static __device__ __forceinline__ void StoreFloatPair(float* dst, float2 value)
{
    ...
    Pair packed = static_cast<Pair>(__float_as_uint(value.x))
                  | (static_cast<Pair>(__float_as_uint(value.y)) << 32);
...
template<int Stripe>
static __device__ __forceinline__ void PublishDiagonalStripe(float const (&gram)[32],
                                                              float beta0,
                                                              float beta1,
                                                              int t0,
                                                              int t1,
...
template<int Stripe>
static __device__ __forceinline__ void PublishSubdiagonalStripe(float const (&gram)[32],
                                                                 float beta0,
                                                                 float beta1,
                                                                 float* dst,
                                                                 int pair_base)
...
template<int Stripe>
static __device__ __forceinline__ void PublishLowerLeftStripe(float const (&gram)[32],
                                                               float beta0,
                                                               float beta1,
                                                               float* dst,
                                                               int row_word_base,
...
struct Tf32LoadTransform {
    ...
    if (wg_idx == 0) {
        ...
        auto  s_a32i0_n =
            cute::make_tensor(cute::make_smem_ptr(scratch.a32i0), TailMmaBColMajorLayout());
        ...
        auto s_a32tmp = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<float*>(scratch.a16i)),
                                          TailMmaALayout());
        ...
        auto t_gram_fragment = cute::make_tensor(cute::make_rmem_ptr(gram_fragment),
        ...
        if (role_tid == 0) {
            ...
            // Keep raw TMA issue here for the same audited fast-codegen reason documented at descriptor creation.
...
#pragma unroll
        for (int k_block = 0; k_block < K_BLOCK_MAX; ++k_block) {
            cute::gemm(gmma64, tCrK(cute::_, cute::_, k_block), tCrKt(cute::_, cute::_, k_block), t_gram_fragment);
        ...
        for (int group = ConsumerWg; group < groups_per_k_head; group += kKktSolveConsumerWgs) {
            ...
            auto s_a64_tail = cute::make_tensor(cute::make_smem_ptr(a64 + kA64LowerHalfOffset),
                                                Swizzled64Lower32Layout());
            if (group >= 2) {
                if (role_tid == 0) {
            ...
            switch (warp) {
                case 0: {
                    ...
                    PublishDiagonalStripe<0>(
                        gram_fragment, beta0, beta1, t0, t1, scratch.a16i, diag_base);
                    PublishDiagonalStripe<1>(
                        gram_fragment, beta0, beta1, t0, t1, scratch.a16i, diag_base);
                    break;
                }
                case 1: {
                    const int subdiag_base = Block16PairBaseOffset(0, t1, 2 * t0);
                    PublishSubdiagonalStripe<0>(
                        gram_fragment, beta0, beta1, scratch.a16o, subdiag_base);
                    PublishSubdiagonalStripe<1>(
                        gram_fragment, beta0, beta1, scratch.a16o, subdiag_base);

                    const int diag_base = Block16PairBaseOffset(1, t1, 2 * t0);
                    PublishDiagonalStripe<2>(
                        gram_fragment, beta0, beta1, t0, t1, scratch.a16i, diag_base);
                    PublishDiagonalStripe<3>(
                        gram_fragment, beta0, beta1, t0, t1, scratch.a16i, diag_base);
                    break;
                }
                case 2: {
                    ...
                    PublishDiagonalStripe<4>(
                        gram_fragment, beta0, beta1, t0, t1, scratch.a16i, diag_base);
                    PublishDiagonalStripe<5>(
                        gram_fragment, beta0, beta1, t0, t1, scratch.a16i, diag_base);
                    break;
                }
                default: {
                    ...
                    PublishSubdiagonalStripe<4>(
                        gram_fragment, beta0, beta1, scratch.a16o, subdiag_base);
                    PublishSubdiagonalStripe<5>(
                        gram_fragment, beta0, beta1, scratch.a16o, subdiag_base);

                    const int diag_base = Block16PairBaseOffset(3, t1, 2 * t0);
                    PublishDiagonalStripe<6>(
                        gram_fragment, beta0, beta1, t0, t1, scratch.a16i, diag_base);
                    PublishDiagonalStripe<7>(
                        gram_fragment, beta0, beta1, t0, t1, scratch.a16i, diag_base);
                    break;
                }
            }
...
#pragma unroll
...

        {
            ...
            auto tA = make_tensor(make_rmem_ptr(a_fragment),
                                  partition_shape_A(mma16, Shape<_16, _16>{}));
            auto tB = make_tensor(make_rmem_ptr(b_fragment),
                                  partition_shape_B(mma16, Shape<_16, _16>{}));
            auto t_block = make_tensor(make_rmem_ptr(block_fragment),
                                       partition_shape_C(mma16, Shape<_16, _16>{}));

            clear(t_block);
#pragma unroll
            for (int i = 0; i < size(tA); ++i) {
                const auto coord = tAcA(i);
                tA(i) = Tf32LoadTransform{}(
                    s_a16i(pair_id * 2 + 1, get<0>(coord), get<1>(coord)));
            }
#pragma unroll
            for (int i = 0; i < size(tB); ++i) {
                const auto coord = tBcB(i);
                tB(i) = Tf32LoadTransform{}(
                    s_a16o(pair_id, get<1>(coord), get<0>(coord)));
            }
#pragma unroll
...
#pragma unroll
            for (int row_select = 0; row_select < 2; ++row_select) {
                ...
                const float2 value = make_float2(block_fragment[fragment_base],
                                                 block_fragment[fragment_base + 1]);
...
#pragma unroll
            for (int i = 0; i < size(tA); ++i) {
                const auto coord = tAcA(i);
                tA(i) = Tf32LoadTransform{}(
                    s_a16o(pair_id, get<0>(coord), get<1>(coord)));
            }
#pragma unroll
            for (int i = 0; i < size(tB); ++i) {
                const auto coord = tBcB(i);
                tB(i) = Tf32LoadTransform{}(
                    s_a16i(pair_id * 2, get<1>(coord), get<0>(coord)));
            }
#pragma unroll
            for (int k_block = 0; k_block < size<2>(tA); ++k_block) {
                cute::gemm(mma16, tA(_, _, k_block), tB(_, _, k_block), t_block);
...
#pragma unroll
            for (int row_select = 0; row_select < 2; ++row_select) {
                ...
                const float2 value = make_float2(block_fragment[fragment_base],
                                                 block_fragment[fragment_base + 1]);
                float* fp32_dst = pair_id == 0 ? &s_a32i0(kBlock16 + row, col)
                                               : &s_a32i1(kBlock16 + row, col);
                StoreFloatPair(fp32_dst, value);
                StoreMmaFragment2(&s_a64(pair_id * kHalfChunk + kBlock16 + row,
                                         pair_id * kHalfChunk + col),
                                  value);
            }
        }
...
#pragma unroll
            ...
            const int  row_base     = 2 * cute::get<0>(row_coord) + cute::get<1>(row_coord)
                                     + 8 * cute::get<2>(row_coord);
...
#pragma unroll
                for (int row_select = 0; row_select < 2; ++row_select) {
                    ...
                    const float2 pair = make_float2(
                        __shfl_sync(0xffffffffu, c_fragment[fragment_base], source_lane),
        ...
        if (role_tid == 0) {
            ...
            cute::SM90_TMA_STORE_4D::copy(
                resolvent_desc, a64, 0, value_head, token0, batch_id);
                         ...
                         cudaStream_t stream)
{
    ...
    Sm90KktSolveKernel<K, ConsumerThreads, ConsumerRegisters>
        <<<grid, Kernel::kKktSolveConsumerWgs * ConsumerThreads, shared_bytes, stream>>>(
        beta_ptr,
        offsets_ptr,
        finished_ptr,
        desc_workspace,
