...
{
    const double scaled =
        std::sqrt(double(problem.hv) * double(std::max(1, problem.total_chunks)) / double(problem.sm_count)) * 3.0;
...
Sm90ContextParallelWorkspace PartitionSm90ContextParallelWorkspace(const Arguments& args, const Plan& plan)
{
    ...
    const auto partition =
        [&](size_t& layout_offset, core::Tensor& tensor, const TensorPlan& tensor_plan, DataType dtype, size_t count) {
    ...
    partition(out.layout.cp_q_offsets_offset, out.cp_q_offsets, cp.cp_q_offsets, kInt32, cp.total_segments + 1);
    partition(
        out.layout.cp_source_indices_offset, out.cp_source_indices, cp.cp_source_indices, kInt32, cp.total_segments);
    partition(out.layout.cp_sequence_starts_offset,
              out.cp_sequence_starts,
              cp.cp_sequence_starts,
              kInt32,
              problem.sequence_num + 1);
    partition(out.layout.cp_state_ptrs_offset, out.cp_state_ptrs, cp.cp_state_ptrs, kInt64, cp.total_segments);
    partition(out.layout.cp_finished_offset, out.cp_finished, cp.cp_finished, kBool, cp.total_segments);
    partition(out.layout.cp_fallback_offset, out.cp_fallback, cp.cp_fallback, kBool, cp.total_segments * problem.hv);
