// Inspired by https://github.com/QwenLM/FlashQLA/blob/60f81453143e724bcaf3fc7921e71e7328f6ebcd/flash_qla/ops/gated_delta_rule/chunk/hopper/prepare_h.py
...
struct Sm90FusedGdrH {
    ...
    static __device__ __forceinline__ WarmupMetadata ComputeWarmupMetadata(
    ...
    static __device__ __forceinline__ void LoadKTile(const CUtensorMap* desc,
                                                     cute::uint64_t*    mbar,
                                                     T*                 smem_tile,
                                                     int                qk_head,
                                                     int                token0)
    {
        ...
        cute::SM90_TMA_LOAD_5D::copy(desc,
                                     mbar,
                                     kTmaNoCacheHint,
                                     smem_tile + kQkHalfElements,
                                     0,
                                     1,
                                     qk_head,
                                     token0,
                                                         ...
                                                         int value_head)
    {
        ...
        for (int row = role_tid - 64; row < kChunkSize; row += kCudaWarpThreads) {
            ...
            cute::SM80_CP_ASYNC_CACHEALWAYS_ZFILL<float>::copy(beta[offset],
                                                               gate_stage[stage][1][row][gate_lane],
                                                               row < valid);
        }
    }
                                                ...
                                                int64_t gate_batch_stride)
{
            ...
            for (int chunk = 0; chunk < chunks; ++chunk) {
                ...
                if (role_tid == 0) {
                    ...
                    cutlass::arch::ClusterTransactionBarrier::expect_transaction(
                        &smem.stage_ready_mbar[stage], kQkTmaBytes);
                    LoadKTile(
                    ...
                    cutlass::arch::ClusterTransactionBarrier::expect_transaction(
                        &smem.stage_ready_mbar[stage], kValueTmaBytes + kSquareTmaBytes);
    ...
    if (wg_idx == 0) {
        ...
        for (int chunk = 0; chunk < chunks; ++chunk) {
            ...
            auto s_x_t = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.q_stage[stage][0][0])),
            ...
            auto s_y_t = cute::make_tensor(cute::make_smem_ptr(y_stage),
                                           FusedGdrGmmaVdTLayout<Element, kWideGdrBlockDv>());
        ...
        auto g_state = cute::make_tensor(
            cute::make_gmem_ptr(segment_state_head),
    ...
    if (wg_idx == 1) {
        ...
        auto s_x_c = cute::make_tensor(
            cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.q_stage[0][0][0])),
        ...
        for (int chunk = 0; chunk < chunks; ++chunk) {
            ...
            auto s_a_t = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.a_stage[stage][0][0])),
            ...
            auto s_k_x = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.k_stage[stage][0][0])),
                                           FusedGdrGmmaQkTransposeALayout<Element>());
            FusedGdrGmmaSs<false>(x_mma, role_tid, s_a_t, s_k_x, tCrX, cute::SM90::GMMA::ScaleOut::Zero);

            auto s_x_store = cute::make_tensor(cute::make_smem_ptr(reinterpret_cast<Element*>(&smem.q_stage[stage][0][0])),
                                               FusedGdrGmmaQkKLayout<Element>());
            #pragma unroll
        ...
        if constexpr (CalcM) {
            ...
            __nv_bfloat16* segment_m_head = segment_m
                                          + (static_cast<int64_t>(segment_id) * hv + value_head) * kHeadDim * kHeadDim
                                          + kFusedGdrBlockDv;
...
template<class T>
__global__ __launch_bounds__(Sm90FusedGdrH<T>::kThreads,
                             Sm90FusedGdrH<T>::kMinBlocks) void
Sm90FusedGdrHKernel(const CUtensorMap* __restrict__ tma_desc_workspace,
                             const float* __restrict__ g_cumsum,
                             const float* __restrict__ beta,
                             __nv_bfloat16* __restrict__ segment_state,
                             __nv_bfloat16* __restrict__ segment_m,
...
void SetFusedGdrHSharedMemoryLimit(size_t smem_bytes)
{
    static const cudaError_t status =
        cudaFuncSetAttribute(Sm90FusedGdrHKernel<__nv_bfloat16>,
                                                  ...
                                                  cudaStream_t        stream)
{
    ...
    Sm90FusedGdrHKernel<__nv_bfloat16><<<grid, block, smem_bytes, stream>>>(
