                                           ...
                                           cudaStream_t               stream)
{
    ...
    if (context_parallel) {
        ...
        correct_initial_states_segment_state_desc =
            MakeContextParallelStateTmaDesc(segment_state_ptr, cp.total_segments, problem.hv, prefix_block_dv);
    ...
    const int head_quads       = (problem.hv + Kernel::kSetupHeadsPerScan - 1) / Kernel::kSetupHeadsPerScan;
