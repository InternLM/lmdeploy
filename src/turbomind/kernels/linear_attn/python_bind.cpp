...
core::Tensor* OptionalTensorPtr(const py::object& object,
                                const char* name,
...
py::dict TensorPlanToDict(const TensorPlan& plan)
{
    ...
    out["storage_size"] = plan.storage_size != 0 ? plan.storage_size
                                                  : static_cast<size_t>(plan.layout.cosize());
    return out;
}
...
Operation MakeOperation(GdrMode mode,
                        std::optional<int> chunk_size,
                        ContextParallelMode cp_mode)
{
                                    ...
                                    int requested_heads_per_block)
{
    ...
    context.heads_per_block = requested_heads_per_block == 0 ? context.hv
                                                              : requested_heads_per_block;
    if (mode == GdrMode::kChunked) {
        context.q_offsets.resize(q_offsets.size());
        TM_CUDA_CHECK(cudaMemcpy(context.q_offsets.data(),
                                 q_offsets.raw_data(),
                                 q_offsets.byte_size(),
...
core::Tensor PromoteStatePointers(core::Tensor state_ptrs)
{
    ...
    if (state_ptrs.ndim() == 1) {
        ...
        return core::Tensor{
            state_ptrs.buffer(),
            core::Layout{{1, sequences, 1},
                         {sequences * state_ptrs.stride(0), state_ptrs.stride(0), 1}},
                    ...
                    int heads_per_block)
{
    ...
    const auto operation = MakeOperation(
        parsed_mode, chunk_size, ParseContextParallelMode(cp_mode));
