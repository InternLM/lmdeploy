...
static __global__ void softmax(T* logits, const int vocab_size_padded, const int* kept)
{
    ...
    float sum_val = 0.0f;
    for (int tid = threadIdx.x; tid < vocab_size_padded; tid += blockDim.x) {
        logits[tid] = __expf((float)logits[tid] - s_max_val);
    ...
    for (int tid = threadIdx.x; tid < vocab_size_padded; tid += blockDim.x) {
        logits[tid] = ((float)logits[tid] / (s_sum_val + 1e-6f));
    }
}
                               ...
                               const float* min_ps)
{
    ...
    if (min_p != 0.f) {
        ...
        BlockPrefixCallbackOp prefix_op(0);
        // minp
        float scaled_min_p = (float)sorted_logits[0] / sum_logits * min_p;
        int   end          = ((n + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
        float prefix_sum   = 0.f;
        for (int i = tid; i < end; i += BLOCK_SIZE) {
            float thread_count = (i < n) ? (float)sorted_logits[i] / sum_logits : 0.f;
