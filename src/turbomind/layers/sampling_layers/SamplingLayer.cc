...
#include "src/turbomind/kernels/sampling_utils.h"
#include "src/turbomind/utils/memory_utils.h"

namespace turbomind {

template<typename T>
void get_min_max_val(const std::vector<T>& vals, T& min_val, T& max_val, T _min, T _max)
{
    min_val = _max;
    max_val = _min;
    for (auto& v : vals) {
...
void SamplingLayer<T>::allocateBuffer(const size_t batch_size)
{
    ...
    logits_ =
        reinterpret_cast<T*>(allocator_->reMalloc(logits_, batch_size * sizeof(T) * args_.vocab_size_padded, false));
...

    {
        ...
        int min_top_k;
        int max_top_k;
        get_min_max_val(runtime_top_k_, min_top_k, max_top_k, 0, 1024);
...
void SamplingLayer<T>::forward(TensorMap* output_tensors, TensorMap* input_tensors)
{
    ...
    int min_topk;
    int max_topk;
    get_min_max_val(runtime_top_k_, min_topk, max_topk, 0, 1024);

    float min_topp;
    float max_topp;
    get_min_max_val(runtime_top_p_, min_topp, max_topp, 0.f, 1.f);

    float min_minp;
    float max_minp;
    get_min_max_val(runtime_min_p_, min_minp, max_minp, 0.f, 1.f);
    ...
    if (min_topk == 0) {
        invokeSoftMax<T>(logits.getPtr<T>(), args_.vocab_size_padded, batch_size, kept_, stream_);
