...
void TopPSamplingLayer<T>::setup(const size_t batch_size, const size_t beam_width, TensorMap* runtime_args)
{
    ...
    **/
    ...
    if (h_runtime_top_k_.size() < batch_size) {
        ...
        h_runtime_min_p_.resize(batch_size);
    }
    ...
    if (runtime_min_p_size > 0) {
        ...
        std::copy_n(runtime_min_p.getPtr<float>(), batch_size, h_runtime_min_p_.data());
    }
    else {
        std::fill_n(h_runtime_min_p_.data(), batch_size, 0.0f);
    }
    ...
    cudaAutoCpy(runtime_min_p_buf_, h_runtime_min_p_.data(), batch_size, stream_);
