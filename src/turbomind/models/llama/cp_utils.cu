...
template<typename T>
__global__ void CpReduce(
    T* out, float* O, float* M, float* L, int token_num, int head_num, int size_per_head, int cp_size, float exp_scale)
{
                    ...
                    cudaStream_t stream)
{
    ...
    CpReduce<<<grid, block, smem_size, stream>>>(out, O, M, L, token_num, head_num, size_per_head, cp_size, exp_scale);
