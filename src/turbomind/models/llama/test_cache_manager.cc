...
size_t LinearSnapshotBytesPerBlock(const ModelParam& model, int attn_tp_size)
{
    ...
    const size_t recurrent_bytes = static_cast<size_t>(num_linear_layers) * num_v_heads * key_head_dim * value_head_dim
                                   * byte_size(model.linear_state_dtype);
...
Sequence
MakeActiveSequence(uint64_t id, const std::vector<int>& prompt, const BlockIds& block_ids, const UniqueIds& unique_ids)
{
    Sequence seq{id};
...
TEST_CASE("BlockTrie caches and matches aligned linear snapshots", "[prefix_cache][linear][trie]")
{
    ...
    BlockTrie::SnapshotValidator validator = [&](int slot, uint64_t unique_id) {
        return slot >= 0 && slot < (int)snapshot_unique_ids.size() && unique_id != 0
               && snapshot_unique_ids[slot] == unique_id;
    };
          ...
          "[prefix_cache][linear][sequence_manager]")
{
    ...
    SequenceManager manager(
        model, kFloat, cache_block_seq_len, attn_tp_size, max_batch_size, 8, -1, true, 1, 1, ctx.device_allocator, [] {
            return size_t{1 << 20};
        });
    ...
    s1.staged_recurrent_snapshots = {
        {2, manager.num_linear_layers_, manager.num_v_heads_, manager.key_head_dim_, manager.value_head_dim_},
          ...
          "[prefix_cache][linear][sequence_manager]")
{
    ...
    REQUIRE(uid != 0);
    Tensor host_conv{
        manager.pooled_prefix_conv_snapshots_.slice(slot, 1).squeeze(0).shape(), manager.linear_conv_dtype_, kCPU};
          ...
          "[prefix_cache][linear][sequence_manager]")
{
    ...
    SequenceManager manager(
        model, kFloat, cache_block_seq_len, attn_tp_size, max_batch_size, 2, -1, true, 1, 1, ctx.device_allocator, [] {
            return size_t{1 << 20};
        });
