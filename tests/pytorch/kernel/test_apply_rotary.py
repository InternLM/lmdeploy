...
class TestApplyRotary:
    ...
    def q_states(self, seq_length, num_heads, feature_dim):
        yield torch.rand(seq_length.sum(), num_heads, feature_dim, device='cuda')
    ...
    def test_apply_rotary(self, q_states, k_states, cached_cos, cached_sin, position_ids_1d, gt):
        q_embed, k_embed = apply_rotary_pos_emb(q_states, k_states, cached_cos, cached_sin, None, position_ids_1d)
        q_gt, k_gt = gt
