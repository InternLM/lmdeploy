...
class TestDSIndex:
    ...
    def test_fp8_index_trim_causal_tail_with_raw_lengths(self, num_heads, head_dim, block_size, device):
        ...
        from lmdeploy.pytorch.backends.cuda.nsa import _get_dsa_index_cache_views
        from lmdeploy.pytorch.consts import dsa_packed_index_cache_shape

        packed_cache = torch.zeros(batch_size * max_num_blocks,
                                   *dsa_packed_index_cache_shape(block_size, head_dim),
                                   dtype=torch.uint8,
                                   device=device)
        packed_values, packed_scales = _get_dsa_index_cache_views(packed_cache, head_dim)
