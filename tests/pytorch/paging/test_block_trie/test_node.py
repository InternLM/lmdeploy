...
def _make_node(block_hash: int):
    return Node(block_hash=block_hash,
                block_id=block_hash,
                token_ids=np.array([block_hash], dtype=np.int64))
...
def test_attached_node_cannot_move_to_another_parent():
    ...
    assert old_root.children[child.block_hash] is child
...
def test_attach_does_not_replace_an_existing_child():
    ...
    replacement = _make_node(displaced.block_hash)
    ...
    assert root.children[displaced.block_hash] is displaced
