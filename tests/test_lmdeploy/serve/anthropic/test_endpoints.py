...
class _FakeServerContext:
    ...
    @staticmethod
    def get_session(_session_id: int):
        return _FakeSession()


def _make_client() -> TestClient:
    app = FastAPI()
    app.include_router(create_anthropic_router(_FakeServerContext))
...
def test_messages_rejects_tools_temporarily():
    client = _make_client()
    response = client.post(
        '/v1/messages',
        headers={'anthropic-version': '2023-06-01'},
    ...
    assert response.status_code == 400
    assert 'temporarily unsupported' in response.json()['error']['message']
