...
from lmdeploy.chat_template import CHAT_TEMPLATES, SamplingParam


def test_base_model():
    model = CHAT_TEMPLATES.get('llama')()
    assert model is not None
    assert model.capability == 'chat'
    assert model.get_prompt('test') == 'test'
    assert model.stop_words is None

    model = CHAT_TEMPLATES.get('internlm')(capability='completion')
...
def test_vicuna():
    ...
    model = CHAT_TEMPLATES.get('vicuna')(capability='completion')
    ...
    model = CHAT_TEMPLATES.get('vicuna')(capability='chat',
                                         system='Provide answers in Python')
    assert model.get_prompt(prompt, sequence_start=True) != prompt
    assert model.get_prompt(prompt, sequence_start=False) != prompt
    assert model.system == 'Provide answers in Python'

    model = CHAT_TEMPLATES.get('vicuna')(capability='voice')
    _prompt = None
...
def test_internlm_chat():
    prompt = 'hello, can u introduce yourself'
    model = CHAT_TEMPLATES.get('internlm-chat-7b')(capability='completion')
    assert model.get_prompt(prompt, sequence_start=True) == prompt
    assert model.get_prompt(prompt, sequence_start=False) == prompt
    ...
    model = CHAT_TEMPLATES.get('internlm-chat-7b')(
        capability='chat', system='Provide answers in Python')
    assert model.get_prompt(prompt, sequence_start=True) != prompt
    assert model.get_prompt(prompt, sequence_start=False) != prompt
    assert model.system == 'Provide answers in Python'

    model = CHAT_TEMPLATES.get('internlm-chat-7b')(capability='voice')
    _prompt = None
    with pytest.raises(AssertionError):
        _prompt = model.get_prompt(prompt, sequence_start=True)
    assert _prompt is None

    model = CHAT_TEMPLATES.get('internlm-chat-7b-8k')()
    assert model.session_len == 8192


def test_baichuan():
    prompt = 'hello, can u introduce yourself'
    model = CHAT_TEMPLATES.get('baichuan-7b')(capability='completion')
    assert model.get_prompt(prompt, sequence_start=True) == prompt
    assert model.get_prompt(prompt, sequence_start=False) == prompt
    assert model.stop_words is None
    assert model.repetition_penalty == 1.1

    model = CHAT_TEMPLATES.get('baichuan-7b')(capability='chat')
    _prompt = model.get_prompt(prompt, sequence_start=True)
...
def test_llama2():
    prompt = 'hello, can u introduce yourself'
    model = CHAT_TEMPLATES.get('llama2')(capability='completion')
    assert model.get_prompt(prompt, sequence_start=True) == prompt
    assert model.get_prompt(prompt, sequence_start=False) == prompt
    assert model.stop_words is None
    assert model.default_sys_prompt is not None

    model = CHAT_TEMPLATES.get('llama2')(capability='chat',
                                         system='Provide answers in Python')
    assert model.get_prompt(prompt, sequence_start=True) != prompt
    assert model.get_prompt(prompt, sequence_start=False) != prompt
    assert model.default_sys_prompt == 'Provide answers in Python'

    model = CHAT_TEMPLATES.get('llama2')(capability='voice')
    _prompt = None
    with pytest.raises(AssertionError):
        _prompt = model.get_prompt(prompt, sequence_start=True)
    assert _prompt is None
...
def test_qwen():
    prompt = 'hello, can u introduce yourself'
    model = CHAT_TEMPLATES.get('qwen-7b')(capability='completion')
    assert model.get_prompt(prompt, sequence_start=True) == prompt
    assert model.get_prompt(prompt, sequence_start=False) == prompt
    assert model.stop_words is not None

    model = CHAT_TEMPLATES.get('qwen-7b')(capability='chat')
    assert model.get_prompt(prompt, sequence_start=True) != prompt
    assert model.get_prompt(prompt, sequence_start=False) != prompt

    model = CHAT_TEMPLATES.get('qwen-7b')(capability='voice')
    _prompt = None
    with pytest.raises(AssertionError):
        _prompt = model.get_prompt(prompt, sequence_start=True)
    assert _prompt is None
...
def test_codellama_completion():
    model = CHAT_TEMPLATES.get('codellama')(capability='completion')
    prompt = """\
import socket
...
def test_codellama_infilling():
    model = CHAT_TEMPLATES.get('codellama')(capability='infilling')
...
def test_codellama_chat():
    model = CHAT_TEMPLATES.get('codellama')(capability='chat',
                                            system='Provide answers in Python')
...
def test_codellama_python_specialist():
    model = CHAT_TEMPLATES.get('codellama')(capability='python')
    prompt = """
...
def test_codellama_others():
    model = None
    with pytest.raises(AssertionError):
        model = CHAT_TEMPLATES.get('codellama')(capability='java')
...
def test_sampling_param():
    ...
    model = CHAT_TEMPLATES.get('llama')(top_p=0.1, top_k=10)
